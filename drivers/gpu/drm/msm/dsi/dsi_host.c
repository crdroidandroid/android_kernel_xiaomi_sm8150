				...
				const struct mipi_dsi_msg *msg)
{
	...
	*/
	...
	if (ret < 0) {
		pr_debug("%s: cmd dma tx failed, type=0x%x, data0=0x%x, len=%d, ret=%d\n",
			__func__, msg->type, (*(u8 *)(msg->tx_buf)), len, ret);
		return ret;
	} else if (ret < len) {
		pr_debug("%s: cmd dma tx failed, type=0x%x, data0=0x%x, ret=%d len=%d\n",
				...
				const struct mipi_dsi_msg *msg)
{
	...
	while (!end) {
		...
		if (ret < 0) {
			...
			pr_debug("%s: Read cmd Tx failed, too short: %d\n", __func__, ret);
			return -ECOMM;
		}
