...
static inline ulong smc(ulong r0, ulong r1, ulong r2, ulong r3)
{
	...
	asm volatile(__asmeq("%0", SMC_ARG0) __asmeq("%1", SMC_ARG1) __asmeq(
			     "%2", SMC_ARG2) __asmeq("%3", SMC_ARG3)
			     __asmeq("%4", SMC_ARG0) __asmeq("%5", SMC_ARG1)
				     __asmeq("%6", SMC_ARG2)
					     __asmeq("%7", SMC_ARG3)
						     SMC_ARCH_EXTENSION
...
static ulong trusty_std_call_inner(struct device *dev, ulong smcnr, ulong a0,
				   ulong a1, ulong a2)
{
	ulong ret;
	int retry = 5;
	...
	while (true) {
		...
		dev_dbg(dev,
			"%s(0x%lx 0x%lx 0x%lx 0x%lx) returned busy, retry\n",
...
static ulong trusty_std_call_helper(struct device *dev, ulong smcnr, ulong a0,
				    ulong a1, ulong a2)
{
	ulong ret;
	int sleep_time = 1;
	...
	while (true) {
		...
		if (sleep_time == 256)
			dev_warn(dev,
				 "%s(0x%lx 0x%lx 0x%lx 0x%lx) returned busy\n",
				 __func__, smcnr, a0, a1, a2);
		dev_dbg(dev,
			"%s(0x%lx 0x%lx 0x%lx 0x%lx) returned busy, wait %d ms\n",
...
static void locked_nop_work_func(struct work_struct *work)
{
	...
	if (ret != 0)
		dev_err(s->dev, "%s: SMC_SC_LOCKED_NOP failed %d", __func__,
			ret);
...
static void nop_work_func(struct work_struct *work)
{
	...
	do {
		...
		ret = trusty_std_call32(s->dev, SMC_SC_NOP, args[0], args[1],
					args[2]);
		...
		if (ret == SM_ERR_NOP_INTERRUPTED)
			...
			dev_err(s->dev, "%s: SMC_SC_NOP failed %d", __func__,
				ret);
	} while (next);
...
{
	atomic_notifier_call_chain(&tstate->notifier, TRUSTY_CALL_RETURNED,
				   NULL);
}
