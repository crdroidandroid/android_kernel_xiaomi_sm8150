...
static void *gpool_shm_alloc(size_t size, gfp_t gfp)
{
	...
	if (!vaddr) {
		dev_err(gdev,
			"%s: alloc shm failed, request 0x%zx avail 0x%zx\n",
...
int trusty_shm_init_pool(struct device *dev)
{
	...
	if (shbuf_dev) {
		...
		shm_size = resource_size(&shbuf_dev->buffer) -
			   shbuf_dev->ramconsole_size;
		shm_va =
			(uintptr_t)shbuf_dev->base + shbuf_dev->ramconsole_size;
		...
		trusty_fast_call32(dev, SMC_FC_IOREMAP_PA_INFO,
				   (u32)shm_ioremap_pa,
				   (u32)(shm_ioremap_pa >> 32), (u32)shm_size);
