...
static void check_all_vqs(struct work_struct *work)
{
	uint i;
	struct trusty_ctx *tctx =
		container_of(work, struct trusty_ctx, check_vqs);
	struct trusty_vdev *tvdev;
...
static int trusty_call_notify(struct notifier_block *nb, unsigned long action,
			      void *data)
{
	struct trusty_ctx *tctx;
...
static void kick_vq(struct trusty_ctx *tctx, struct trusty_vdev *tvdev,
...
static void kick_vqs(struct work_struct *work)
{
	...
	struct trusty_vdev *tvdev;
	struct trusty_ctx *tctx =
		container_of(work, struct trusty_ctx, kick_vqs);
...
static bool trusty_virtio_notify(struct virtqueue *vq)
{
	...
	ulong payload = (smc_nr << 32) | (tvdev->notifyid << 16) |
			(u16)tvr->notifyid;
...
static int trusty_load_device_descr(struct trusty_ctx *tctx, void *va,
				    size_t sz)
{
	int ret;
	...
	ret = trusty_call32_mem_buf(tctx->dev->parent, SMC_SC_VIRTIO_GET_DESCR,
...
static void trusty_virtio_reset(struct virtio_device *vdev)
{
	...
	trusty_std_call32(tctx->dev->parent, SMC_SC_VDEV_RESET, tvdev->notifyid,
			  0, 0);
}
...
static struct virtqueue *_find_vq(struct virtio_device *vdev, unsigned id,
...
)
{
	...
	tvr->vq = vring_new_virtqueue(id, tvr->elem_num, tvr->align, vdev, true,
				      tvr->vaddr, trusty_virtio_notify,
				      callback, name);
#elif LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
	tvr->vq = vring_new_virtqueue(id, tvr->elem_num, tvr->align, vdev, true,
				      ctx, tvr->vaddr, trusty_virtio_notify,
				      callback, name);
#endif
	if (!tvr->vq) {
				    ...
				    void *config)
{
	...
	tvdev = kzalloc(sizeof(struct trusty_vdev) +
				vdev_descr->num_of_vrings *
					sizeof(struct trusty_vring),
			GFP_KERNEL);
	if (!tvdev) {
...
static int trusty_parse_device_descr(struct trusty_ctx *tctx, void *descr_va,
				     size_t descr_sz)
{
	u32 i;
