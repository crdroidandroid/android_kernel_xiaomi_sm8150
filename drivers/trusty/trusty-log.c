...
static int trusty_log_probe(struct platform_device *pdev)
{
	...
	result = trusty_std_call32(s->trusty_dev, SMC_SC_SHARED_LOG_ADD,
				   (u32)(pa), (u32)(pa >> 32), TRUSTY_LOG_SIZE);
	if (result < 0) {
...
error_call_notifier:
	trusty_std_call32(s->trusty_dev, SMC_SC_SHARED_LOG_RM, (u32)pa,
			  (u32)(pa >> 32), 0);
error_std_call:
...
static int trusty_log_remove(struct platform_device *pdev)
{
	...
	result = trusty_std_call32(s->trusty_dev, SMC_SC_SHARED_LOG_RM, (u32)pa,
				   (u32)(pa >> 32), 0);
	if (result) {
