			    ...
			    pgprot_t pgprot, bool writable)
{
	...
	return encode_page_inf(inf, (uint64_t)page_to_phys(page), pgprot,
			       writable);
}
			  ...
			  u32 size, pgprot_t pgprot, bool writable)
{
	...
	if (SMC_IS_FASTCALL(smcnr)) {
		return trusty_fast_call32(dev, smcnr, (u32)pg_inf.attr,
					  (u32)(pg_inf.attr >> 32), size);
	} else {
		return trusty_std_call32(dev, smcnr, (u32)pg_inf.attr,
					 (u32)(pg_inf.attr >> 32), size);
	}
}
