...
s32 trusty_link_shbuf_recv(u32 lsdidx, ulong *out)
{
	...
	do {
		...
		if (ret < 0) {
			dev_err(dev->dev,
				"wait failed ret %d virq_pending %u\n", ret,
				dev->virq_pending);
			return ret;
		} else if (ret == 0) {
			dev_err(dev->dev,
				"wait timeout ret %d virq_pending %u\n", ret,
				dev->virq_pending);
			return -ETIMEDOUT;
		}
...
static irqreturn_t link_shbuf_irq_handler(int irq, void *data)
{
	...
	if (payload == TRUSTY_PROBE_DEFER) {
		...
		do {
			...
			new = old | payload;
		} while (atomic64_cmpxchg(&priv->virq_payload, old, new) !=
			 old);
...
static int trusty_link_shbuf_probe(struct platform_device *pdev)
{
	...
	if (link_shbuf_dev_num >= MAX_LINK_SHBUF_DEV) {
		dev_err(&pdev->dev,
			"link-shbuf devices > MAX_LINK_SHBUF_DEV\n");
		return -ENOMEM;
	}
	...
	if (!devm_request_mem_region(&pdev->dev, priv->buffer.start,
		...
		ret = -EINVAL;
		dev_err(&pdev->dev,
			"failed to request_mem_region start %llx size %llx\n",
	...
	if (!priv->base) {
		dev_err(&pdev->dev,
			"failed to memremap, start %llx size %llx\n",
			priv->buffer.start, resource_size(&priv->buffer));
	...
	dev_info(&pdev->dev,
		 "shbuf pa %llx size %llx memremap_va 0x%lx rc_sz %zx\n",
		 priv->buffer.start, resource_size(&priv->buffer),
	...
	if (ret < 0) {
		dev_err(&pdev->dev,
			"failed to read shared memory permissions\n");
		goto err_unmap_dev;
	}
	...
	if (!IS_ERR_OR_NULL(debugfs_root)) {
		debugfs_create_file("shbuf_virq", 0600, debugfs_root, NULL,
				    &shbuf_virq_fops);
		debugfs_create_file("shbuf_buffer", 0600, debugfs_root, priv,
				    &shbuf_buffer_fops);
	} else {
	...
	for (i = 0; i < link_shbuf_dev_num; i++) {
		dev_info(
			&pdev->dev,
			"  link_shbuf_dev[%d] virq %d hwirq %d virqline %d base 0x%lx remap_pa 0x%llx\n",
			i, link_shbuf_dev[i]->virq, link_shbuf_dev[i]->hwirq,
...
static int fifo_vipc_probe(struct platform_device *pdev)
{
	...
	if (ret < 0 || priv->virqline == OKL4_KCAP_INVALID) {
		dev_err(&pdev->dev,
			"failed to get \"reg\" of interrupt-line\n");
		return -ENODEV;
	}
	...
	dev_info(&pdev->dev, "%s: virq %d hwirq %d virqline %d\n", __func__,
		 priv->virq, priv->hwirq, priv->virqline);
