...
static int trusty_irq_create_irq_mapping(struct trusty_irq_state *is, int irq)
{
	...
	for (index = 0;; index += 3) {
		ret = of_property_read_u32_index(is->dev->of_node,
						 "interrupt-ranges", index,
						 &range_base);
		if (ret)
			return ret;

		ret = of_property_read_u32_index(is->dev->of_node,
						 "interrupt-ranges", index + 1,
						 &range_end);
		if (ret)
			return ret;
	...
	ret = of_property_read_u32_index(is->dev->of_node, "interrupt-ranges",
					 index + 2, &templ_idx);
	if (ret)
		return ret;
...
static int trusty_irq_init_normal_irq(struct trusty_irq_state *is, int tirq)
{
	...
	if (irq < 0) {
		dev_err(is->dev, "trusty_irq_create_irq_mapping failed (%d)\n",
			irq);
		return irq;
	}
	...
	ret = request_irq(irq, trusty_irq_handler, IRQF_NO_THREAD, "trusty",
			  trusty_irq);
	if (ret) {
...
{
	return trusty_fast_call32(is->trusty_dev, SMC_FC_GET_NEXT_IRQ, min_irq,
				  per_cpu, 0);
}

static int trusty_irq_init_one(struct trusty_irq_state *is, int irq,
			       bool per_cpu)
{
	int ret;
...
static void trusty_irq_free_irqs(struct trusty_irq_state *is)
{
	...
	hlist_for_each_entry_safe(
		irq, n, &this_cpu_ptr(is->percpu_irqs)->inactive, node) {
		struct trusty_irq __percpu *trusty_irq_handler_data;
