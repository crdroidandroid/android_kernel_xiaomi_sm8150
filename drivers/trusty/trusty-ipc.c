					      ...
					      size_t sz)
{
	...
	if (vds->free_msg_buf_cnt) {
		...
		mb = list_first_entry(&vds->free_buf_list, struct tipc_msg_buf,
				      node);
...
static struct tipc_msg_buf *vds_get_txbuf(struct tipc_virtio_dev *vds,
					  size_t sz, long timeout, bool intr)
{
	struct tipc_msg_buf *mb;
...
static int vds_add_channel(struct tipc_virtio_dev *vds, struct tipc_chan *chan)
{
	...
	if (vds->state == VDS_ONLINE) {
		ret = idr_alloc_cyclic(&vds->addr_idr, chan,
				       TIPC_MIN_LOCAL_ADDR, TIPC_MAX_LOCAL_ADDR,
				       GFP_KERNEL);
		if (ret > 0) {
			chan->local = ret;
...
static void vds_memref_release(struct tipc_virtio_dev *vds, const u32 *ids,
			       uint ids_num)
{
	uint i;
				...
				struct tipc_shmem *shm)
{
	...
	ret = build_kern_page_info(hdr->pg_inf, vma, mref->pages, pg_num);
...
static int append_user_memref(struct tipc_msg_mref_hdr *hdr,
			      struct tipc_memref *mref, struct tipc_shmem *shm)
{
	long ret;
	...
	ret = build_user_page_info(hdr->pg_inf, vmas, mref->pages,
				   mref->pg_pinned);
			  ...
			  bool user)
{
	...
	for (i = 0; i < shmv_cnt; i++) {
		...
		/* create memref id */
		ret = idr_alloc(&vds->mref_idr, mref, TIPC_MIN_MEMREF_ID,
				TIPC_MAX_MEMREF_ID, GFP_KERNEL);
...
static int _tipc_chan_queue_msg(struct tipc_chan *chan, struct tipc_msg_buf *mb,
				...
				bool user)
{
	...
	case TIPC_CONNECTED:
		...
		if (shmv_cnt) {
			err = append_memrefs(chan, mb, mr_ids, shmv, shmv_cnt,
					     user);
			if (err < 0) {
...
int tipc_chan_shutdown(struct tipc_chan *chan)
{
	...
	/* get tx buffer */
	txbuf = vds_get_txbuf(chan->vds, chan->vds->msg_buf_sz, TXBUF_TIMEOUT,
			      false);
	if (IS_ERR(txbuf))
		return PTR_ERR(txbuf);
...
static int dn_wait_for_reply(struct tipc_dn_chan *dn, int timeout)
{
	...
	ret = wait_for_completion_interruptible_timeout(
		&dn->reply_comp, msecs_to_jiffies(timeout));
	if (ret < 0)
		return ret;
				...
				bool compat)
{
	...
	if (compat)
		ret = compat_import_iovec(READ, (void *)(uintptr_t)req.msgiov,
					  req.msgiov_cnt, ARRAY_SIZE(iovstack),
					  &iov, &msg_iter);
	else
		ret = import_iovec(READ, (void *)(uintptr_t)req.msgiov,
				   req.msgiov_cnt, ARRAY_SIZE(iovstack), &iov,
				   &msg_iter);
	if (ret < 0)
		return ret;
	...
	ret = _tipc_send_msg(dn->chan, &msg_iter, shmems, req.shmemv_cnt,
			     mref_malloc, timeout, true);
...
static long tipc_compat_ioctl(struct file *filp, unsigned int cmd,
			      unsigned long arg)
{
	int ret;
	struct tipc_dn_chan *dn = filp->private_data;
			  ...
			  long timeout, bool intr)
{
	...
	if (shmv_cnt) {
		...
		aux_len =
			aux_data_len(shmv, shmv_cnt) + ALIGN(len, MEMREF_ALIGN);
	}
	...
	list_for_each_entry_safe(mshm, mshmtmp, &dn->mref_shm_list, node) {
		if (mshm->vm_start == vma->vm_start) {
			pr_info("%s: uva %lx sz %zx fl %lx kva %lx kpa %llx\n",
				__func__, mshm->vm_start, mshm->size,
				vma->vm_flags, mshm->vaddr, mshm->paddr);
			_free_shareable_mem(mshm->size, (void *)mshm->vaddr,
					    mshm->paddr);
			list_del(&mshm->node);
			kfree(mshm);
			found = true;
...
static int tipc_mref_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	pr_info("%s: uva %lx sz %zx fl %lx kva %lx kpa %llx\n", __func__,
		mshm->vm_start, mshm->size, vma->vm_flags, mshm->vaddr,
		mshm->paddr);
...
static int _create_cdev_node(struct device *parent, struct tipc_cdev_node *cdn,
			     const char *name)
{
	int ret;
	dev_t devt;
	...
	cdn->dev = device_create(tipc_class, parent, devt, NULL,
				 "trusty-ipc-%s", name);
	if (IS_ERR(cdn->dev)) {
		ret = PTR_ERR(cdn->dev);
...
static void _handle_ctrl_msg(struct tipc_virtio_dev *vds, void *data, int len,
			     u32 src)
{
	...
	if ((len < sizeof(*msg)) || (sizeof(*msg) + msg->body_len != len)) {
		dev_err(&vds->vdev->dev,
			...
			(int)(sizeof(*msg) + msg->body_len), len);
		return;
	}
	...
	default:
		dev_warn(&vds->vdev->dev, "%s: Unexpected message type: %d\n",
			 __func__, msg->type);
	}
}
...
static void _discard_chan_msg(struct tipc_virtio_dev *vds,
			      struct tipc_msg_hdr *msg, struct tipc_msg_buf *mb)
{
