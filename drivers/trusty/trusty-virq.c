...
s32 trusty_virq_recv(u32 virqidx, ulong *out)
{
	...
	dev_dbg(dev->dev, "%s: irq %d hwirq %d payload 0x%lx\n", __func__,
		dev->virq.irqno, dev->virq.hwirq, payload);
...
static int trusty_virq_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, vdev->virq.irqno, virq_handler, 0,
			       "virq", vdev);
	if (ret < 0) {
	...
	for (i = 0; i < virqdev_num; i++) {
		dev_info(&pdev->dev,
			 "  virqdev[%d] irqno %d hwirq %d kcap %d\n", i,
			 virqdev[i]->virq.irqno, virqdev[i]->virq.hwirq,
