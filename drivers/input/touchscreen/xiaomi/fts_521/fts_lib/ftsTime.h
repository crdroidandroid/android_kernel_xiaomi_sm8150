...
#define TIMEOUT_RESOLUTION \
	2 /*timeout resolution in ms (all timeout should be multiples of this unit)*/
#define GENERAL_TIMEOUT (50 * TIMEOUT_RESOLUTION) /*general timeout in ms*/
#define RELEASE_INFO_TIMEOUT \
	(15 * TIMEOUT_RESOLUTION) /* timeout to request release info in ms*/

#define TIMEOUT_REQU_COMP_DATA \
	(100 *                 \
	 TIMEOUT_RESOLUTION) /*timeout to request compensation data in ms*/
#define TIMEOUT_REQU_DATA \
	(100 * TIMEOUT_RESOLUTION) /*timeout to request data in ms*/
#define TIMEOUT_ITO_TEST_RESULT \
	(100 * TIMEOUT_RESOLUTION) /*timeout to perform ito test in ms*/
#define TIMEOUT_INITIALIZATION_TEST_RESULT \
	(5000 *                            \
	 TIMEOUT_RESOLUTION) /*timeout to perform initialization test in ms*/
#define TIMEOUT_ECHO \
	TIMEOUT_INITIALIZATION_TEST_RESULT /*timeout of the echo command, should be the max of all the possible commands (used in worst case)*/
/** @}*/
