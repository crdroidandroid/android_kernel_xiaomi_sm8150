...
#define ERROR_TIMEOUT \
	((int)0x80000007) /*timeout expired! exceed the max number of retries or the max waiting time*/
#define ERROR_FILE_NOT_FOUND \
	((int)0x80000008) /*the file that i want to open is not found*/
...
#define ERROR_FW_NO_UPDATE \
	((int)0x8000000C) /*fw in the chip newer than the one in the memmh*/
...
#define ERROR_DISABLE_INTER \
	((int)0x80000200) /*unable to disable the interrupt*/
...
#define ERROR_GET_OFFSET \
	((int)0x80000500) /*unable to read an offset from memory*/
#define ERROR_GET_FRAME_DATA \
	((int)0x80000600) /*unable to retrieve the data of a required frame*/
#define ERROR_DIFF_DATA_TYPE \
	((int)0x80000700) /*FW answers with an event that has a different address respect the request done*/
#define ERROR_WRONG_DATA_SIGN \
	((int)0x80000800) /*the signature of the host data is not HEADER_SIGNATURE*/
#define ERROR_SET_SCAN_MODE_FAIL \
	((int)0x80000900) /*setting the scanning mode failed (sense on/off etc...)*/
#define ERROR_SET_FEATURE_FAIL \
	((int)0x80000A00) /*setting a specific feature failed*/
#define ERROR_SYSTEM_RESET_FAIL \
	((int)0x80000B00) /*the comand SYSTEM RESET failed*/
#define ERROR_FLASH_NOT_READY \
	((int)0x80000C00) /*flash status not ready within a timeout*/
#define ERROR_FW_VER_READ \
	((int)0x80000D00) /*unable to retrieve fw_vers or the config_id*/
#define ERROR_GESTURE_ENABLE_FAIL \
	((int)0x80000E00) /*unable to enable/disable the gesture*/
#define ERROR_GESTURE_START_ADD \
	((int)0x80000F00) /*unable to start to add custom gesture*/
#define ERROR_GESTURE_FINISH_ADD \
	((int)0x80001000) /*unable to finish to add custom gesture*/
#define ERROR_GESTURE_DATA_ADD \
	((int)0x80001100) /*unable to add custom gesture data*/
#define ERROR_GESTURE_REMOVE \
	((int)0x80001200) /*unable to remove custom gesture data*/
#define ERROR_FEATURE_ENABLE_DISABLE \
	((int)0x80001300) /*unable to enable/disable a feature mode in the IC*/
#define ERROR_NOISE_PARAMETERS \
	((int)0x80001400) /*unable to set/read noise parameter in the IC*/
#define ERROR_CH_LEN \
	((int)0x80001500) /*unable to retrieve the force and/or sense length*/
/** @}*/
...
#define ERROR_REQU_COMP_DATA \
	((int)0x80010000) /*compensation data request failed*/
#define ERROR_REQU_DATA ((int)0x80020000) /*data request failed*/
#define ERROR_COMP_DATA_HEADER \
	((int)0x80030000) /*unable to retrieve the compensation data   header*/
#define ERROR_COMP_DATA_GLOBAL \
	((int)0x80040000) /*unable to retrieve the global compensation data*/
#define ERROR_COMP_DATA_NODE \
	((int)0x80050000) /*unable to retrieve the compensation data for each node*/
#define ERROR_TEST_CHECK_FAIL \
	((int)0x80060000) /*check of production limits or of fw answers failed*/
...
#define ERROR_HANDLER_STOP_PROC \
	((int)0x800D0000) /*stop the poll of the FIFO if particular errors are found*/
#define ERROR_CHECK_ECHO_FAIL \
	((int)0x800E0000) /*unable to retrieve echo event*/
...
#define ERROR_PROD_TEST_INITIALIZATION \
	((int)0x84000000) /*production initialization test failed*/
#define ERROR_GET_INIT_STATUS \
	((int)0x85000000) /*mismatch of the MS or SS tuning_version*/
#define ERROR_LOCKDOWN_CODE \
	((int)0x80001600) /*unable to write/rewrite/read lockdown code in the IC*/

#define EVT_TYPE_ERROR_LOCKDOWN_FLASH \
	0x30 /*FW shall not proceed with any flash write/read*/
#define EVT_TYPE_ERROR_LOCKDOWN_CRC \
	0x31 /*FW shall discard the record and do not write to flash*/
#define EVT_TYPE_ERROR_LOCKDOWN_NO_DATA \
	0x32 /*No data of this type exisitng in flash*/
#define EVT_TYPE_ERROR_LOCKDOWN_WRITE_FULL \
	0x33 /*FW shall not write this new record to flash*/
/** @}*/
...
typedef struct {
	u8 list[FIFO_DEPTH *
		FIFO_EVENT_SIZE]; /*byte array which contains the series of error events encountered from the last reset of the list.*/
