...
#define FLASH_STATUS_BYTES \
	1 /*number of bytes to check for read the flash status*/

/*Flash timing parameters*/
#define FLASH_RETRY_COUNT 200 /*number of attemps to read the flash status*/
#define FLASH_WAIT_BEFORE_RETRY \
	50 /*time to wait in ms between one status reading and another*/
...
#define FLASH_CHUNK \
	(64 *       \
	 1024) /*Max number of bytes that the DMA can burn on the flash in one shot in FTI*/
#define DMA_CHUNK \
	32 /*Max number of bytes that can be written in I2C to the DMA*/
...
typedef struct {
	...
	u8 externalRelease
		[EXTERNAL_RELEASE_INFO_SIZE]; /*External Release Info of the FW file*/
