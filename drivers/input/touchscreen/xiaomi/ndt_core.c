...
static ssize_t pressure_pressure_show(struct device *dev,
				      struct device_attribute *attr, char *buf);
static ssize_t ndt_threshold_show(struct device *dev,
				  struct device_attribute *attr, char *buf);
static ssize_t ndt_reset_store(struct device *dev,
			       struct device_attribute *attr, const char *buf,
			       size_t count);
static ssize_t ndt_irq_enable_store(struct device *dev,
				    struct device_attribute *attr,
				    const char *buf, size_t count);
static ssize_t ndt_threshold_store(struct device *dev,
				   struct device_attribute *attr,
				   const char *buf, size_t count);
				       ...
				       struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(buf, PAGE_SIZE,
		       "ic_fw_ver:%02x%02x,file_fw_ver:%02x%02x\n",
		       ic_fw_ver[0], ic_fw_ver[1], file_fw_ver[1],
		       file_fw_ver[0]);
	release_firmware(fw_entry);
	kfree(data);
	data = NULL;
...
int ndt_update_fw(bool force, char *fw_name)
{
	loff_t pos = 0;
	char fw_data_len[4];
	int len = 0;
	...
	i2c_get_clientdata(g_ndt_client);
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	ret = snprintf(
		buf, PAGE_SIZE,
		"device id:%s,manu id:%02x%02x,module id:%02x%02x,fw version:%02x%02x\n",
...
static void ndt_release_work(struct work_struct *work)
{
	struct ndt_force_data *data =
		container_of(work, struct ndt_force_data, release_work);
	int pressure = 0;
	int touch_down, x, y;
...
{
	struct ndt_force_data *data =
		container_of(timer, struct ndt_force_data, release_timer);
...
*/
	...
	if (data->released == 1)
		hrtimer_start(&data->release_timer,
			      ktime_set(0, 15 * 1000 * 1000), HRTIMER_MODE_REL);
	if (!data->released) {
		hrtimer_start_range_ns(&data->release_timer,
				       ns_to_ktime(15 * 1000 * 1000), 0,
				       HRTIMER_MODE_REL);
	}
	data->released = 0;
	return IRQ_HANDLED;
