...
*******************************************************************************/
extern s32 normandy_reg_write(IN PST_TP_DEV p_dev, IN u16 addr, IN u8 *p_buf,
			      IN u16 buf_len)
{
	s32 ret = 0;
	u8 retry_cnt = 3;
...
* Return		: u32(1:ok 0:fail)
*******************************************************************************/
extern s32 normandy_reg_read(IN PST_TP_DEV p_dev, IN u16 addr, IN_OUT u8 *p_buf,
			     IN u16 buf_len)
{
	s32 ret = 0;
	u8 retry_cnt = 3;
...
* Return		: u32(0:fail 1:ok)
*******************************************************************************/
extern s32 normandy_force_update_cfg(IN PST_TP_DEV p_dev, IN u8 *config,
				     IN u16 cfg_len)
{
	s32 ret = 0;
	u8 ver_tmp = 0;
	...
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_read(p_dev, p_dev->cfg_start_addr, &ver_tmp, 1);
		if (1 == ret) {
			break;
		}
	}
				  ...
				  IN u16 cfg_len)
{
		...
		if (1 == ret) {
			...
			for (j = 0; j < retry_cnt1; j++) {
				...
				ret = chip_reg_read(p_dev, p_dev->real_cmd_addr,
						    buf_arr, 1);
				if (1 == ret && write_wait_cmd == buf_arr[0]) {
					break;
				} else {
					ret = 0;
				}
			...
			board_print_error("fail to send write cfg cmd!");
			return 0;
		}
	}
	...
	retry_cnt = 3;
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_write(p_dev, p_dev->cfg_start_addr, config,
				     cfg_len);
		if (1 == ret) {
			break;
		}
...
*******************************************************************************/
extern s32 normandy_read_cfg_arr(IN PST_TP_DEV p_dev, IN_OUT u8 *config,
				 IN u16 cfg_len)
{
	s32 ret = 1;
	u8 buf_arr[10];
	...
	if (cfg_len == 0) { /*config len is not known*/
		/*read config head*/
		for (i = 0; i < retry_cnt; i++) {
			ret = chip_reg_read(p_dev, p_dev->cfg_start_addr,
					    config, 4);
			if (ret == 1) {
				break;
			}
		...
		for (j = 0; j < sub_bags; j++) {
			...
			for (i = 0; i < retry_cnt; i++) {
				ret = chip_reg_read(
					p_dev, p_dev->cfg_start_addr + offset,
					config + offset, 2);
				if (ret == 1) {
					break;
				}
			...
			board_print_debug("read sub bug %d offset is %d\n", j,
					  offset);
...
* Return		: none
*******************************************************************************/
extern void normandy_cfg_update_chksum(IN_OUT u8 *config, IN u16 cfg_len)
{
...
* Return		: u32(0:fail 1:ok)
*******************************************************************************/
extern s32 normandy_cmp_cfg(IN PST_TP_DEV p_dev, IN u8 *cfg1, IN u8 *cfg2,
			    IN u16 cfg_len)
{
	s32 ret = 1;
	u16 i = 0;
...
* Return		: u32(0:fail 1:ok)
*******************************************************************************/
extern s32 normandy_doze_mode(IN PST_TP_DEV p_dev, IN u8 b_ena, IN u8 b_force)
{
	s32 ret = 1;
	u8 temp_data = 0;
	u8 temp_read = 0;
	...
	if (b_ena) { /*enter doze*/
		...
		if (p_dev->doze_ref_cnt == 0 || b_force) {
			temp_data = 0xCC;
			for (i = 0; i < retry_cnt; i++) {
				ret = board_write_chip_reg(p_dev, 0x30f0,
							   &temp_data, 1);
				if (1 == ret) {
					break;
				}
		...
		if (p_dev->doze_ref_cnt == 1 || b_force) {
			for (i = 0; i < retry_cnt; i++) {
				if (0xAA !=
				    temp_read) { /*only 0x3100 is not 0xAA and need write 0x30f0 0xAA */
					temp_data = 0xAA;
					ret = board_write_chip_reg(
						p_dev, 0x30f0, &temp_data, 1);
					if (ret == 0) {
						continue;
					}
					board_delay_ms(1);
				...
				for (j = 0; j < retry_cnt; j++) {
					ret = board_read_chip_reg(
						p_dev, 0x3100, &temp_read, 1);
					if ((1 == ret && 0xBB == temp_read)) {
						break;
					}
					if ((1 == ret && 0xAA != temp_read)) {
						ret = 0;
