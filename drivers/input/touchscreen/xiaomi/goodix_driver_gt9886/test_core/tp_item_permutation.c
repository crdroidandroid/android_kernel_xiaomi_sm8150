...
extern s32 get_index_from_permutaion_arr(IN u16 item_id)
{
	...
	s32 i = 0;
	for (i = 0;
	     i < arr_size && cs_item_permutation[i] != TP_TEST_ITEM_END_ID;
	     i++) {
...
*******************************************************************************/
extern s32 sort_item_permutation(IN_OUT PST_TEST_PROC_DATA p_test_proc_data)
{
	s32 ret = 1;
	s32 i = 0, j = 0;
	s32 item_cnt = 0;
	...
	if (NULL != p_test_proc_data) {
		...
		for (i = 0; i < item_cnt; i++) {
			for (j = i + 1; j < item_cnt; j++) {
				index1 = get_index_from_permutaion_arr(
					p_test_proc_data->pp_test_item_arr[i]
						->test_item_id);
				index2 = get_index_from_permutaion_arr(
					p_test_proc_data->pp_test_item_arr[j]
						->test_item_id);
