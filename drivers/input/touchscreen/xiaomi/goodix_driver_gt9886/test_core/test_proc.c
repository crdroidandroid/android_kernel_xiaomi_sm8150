...
extern s32 test_proc_func(IN_OUT PST_TEST_PROC_DATA p_test_proc_data)
{
	...
	if (NULL != p_test_proc_data->p_test_items_before_func_ptr) {
		res &= p_test_proc_data->p_test_items_before_func_ptr(
			p_test_proc_data->p_test_items_before_func_ptr_param);
	}
	...
	for (i = 0; i < p_test_proc_data->test_item_num; i++) {
		...
		if (NULL != p_test_proc_data->pp_test_item_arr[i] &&
		    NULL != p_test_proc_data->pp_test_item_arr[i]
				    ->ptr_test_func) {
	...
	if (NULL != p_test_proc_data->p_test_items_finished_func_ptr) {
		res &= p_test_proc_data->p_test_items_finished_func_ptr(
			p_test_proc_data->p_test_items_finished_func_ptr_param);
	}
...
*******************************************************************************/
extern s32 register_test_item_func(IN_OUT PST_TEST_PROC_DATA p_test_proc_data)
{
	s32 ret = 1;
	u16 i = 0;
	PST_TEST_ITEM *pp_test_item_arr = p_test_proc_data->pp_test_item_arr;
	...
	for (i = 0; i < item_num; i++) {
		...
		case TP_SHORT_TEST_ITEM_ID: {
			pp_test_item_arr[i]->ptr_test_func = short_test_func;
		} break;

			/*----------------------firmware test-----------------------*/
		case TP_VERSION_TEST_ITEM_ID: {
			pp_test_item_arr[i]->ptr_test_func = fw_version_test;
		} break;

			/*----------------------other test-----------------------*/
		case TP_CHIP_CFG_PROC_ITEM_ID: {
			pp_test_item_arr[i]->ptr_test_func = tp_chip_cfg_proc;
		} break;
		case TP_FLASH_TEST_ITEM_ID: {
			pp_test_item_arr[i]->ptr_test_func = tp_flash_test;
		} break;
		...
		default: {
			pp_test_item_arr[i]->ptr_test_func = NULL;
		} break;
		}
	}
	return ret;
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 get_test_item_from_proc_data(IN PST_TEST_PROC_DATA p_test_proc_data,
					IN u16 test_item_id,
					OUT PST_TEST_ITEM *pp_test_item)
{
	u16 i = 0;
	s32 ret = 0;
	PST_TEST_ITEM *pp_test_item_arr = p_test_proc_data->pp_test_item_arr;
...
* Return		: none
*******************************************************************************/
extern void test_before_func_fail_proc(IN_OUT P_TEST_ITEM_RES p_test_res_head)
{
	if (p_test_res_head->test_item_err_code.len >= 1) {
		p_test_res_head->test_item_err_code.ptr_err_code_set[0] =
			TEST_BEFORE_FUNC_FAIL;
	}
	p_test_res_head->test_status = TEST_ABORTED;
	p_test_res_head->test_result = TEST_NG;
	board_print_error("p_test_item->ptr_test_item_before_func fail\n");
}
