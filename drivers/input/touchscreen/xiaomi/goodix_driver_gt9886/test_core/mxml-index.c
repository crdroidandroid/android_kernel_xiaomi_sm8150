...
{ /* I - Attribute to index or @code NULL@ for none */
	...
	if (!element && !attr)
		...
		curr = mxmlFindElement(node, node, element, attr, NULL,
				       MXML_DESCEND);
	...
	while (curr) {
		...
		curr = mxmlFindElement(curr, node, element, attr, NULL,
				       MXML_DESCEND);
	}
...
{ /* I - Second node */
	int diff; /* Difference */
	diff = strcmp(first->value.element.name, second->value.element.name);
	if (diff != 0)
		return diff;
	if (ind->attr) {
