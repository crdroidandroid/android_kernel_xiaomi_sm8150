...
static void _MXML_FINI(void)
{
	...
	if (_mxml_key != -1) {
		global = (_mxml_global_t *)pthread_getspecific(_mxml_key);
		if (global != NULL)
...
{ /* I - Unused */
	...
	case DLL_PROCESS_ATTACH: /* Called on library initialization */
		_mxml_tls_index = TlsAlloc();
		if (_mxml_tls_index == TLS_OUT_OF_INDEXES)
			return FALSE;
		break;

	case DLL_THREAD_DETACH: /* Called when a thread terminates */
		global = (_mxml_global_t *)TlsGetValue(_mxml_tls_index);
		if (global != NULL)
			free(global);
		break;
