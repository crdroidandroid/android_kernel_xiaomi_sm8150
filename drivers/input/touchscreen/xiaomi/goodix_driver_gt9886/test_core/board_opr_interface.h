...
#define board_print_debug(fmt, arg...) \
...
#endif
#define board_print_error(fmt, arg...) \
	pr_err("[GTP-ERR][%s:%d]" fmt "\n", __func__, __LINE__, ##arg)
#define board_print_warning(fmt, arg...) \
	pr_info("[GTP-WARN][%s:%d]" fmt "\n", __func__, __LINE__, ##arg)
#define board_print_info(fmt, arg...) \
...
extern s32 board_set_power_volt(PST_TP_DEV p_dev, u16 volt, u16 aver_len,
				u8 cnt);
extern s32 board_set_comm_volt(PST_TP_DEV p_dev, u16 volt, u16 aver_len,
			       u8 cnt);
...
extern s32 board_chip_hid_opr(PST_TP_DEV p_dev,
			      s32 b_ena); /*use for hid protocol*/
