...
s32 alloc_test_item_res(PST_TEST_ITEM *pp_test_item, u16 item_num)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		case TP_RAWDATA_TEST_ITEMS_SET_ID: {
			if (1 == alloc_rawdata_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_DIFFDATA_TEST_ITEMS_SET_ID: {
			if (1 == alloc_diffdata_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
		...
		case TP_SHORT_TEST_ITEM_ID: {
			if (1 == alloc_short_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
		...
		case TP_VERSION_TEST_ITEM_ID: {
			if (1 == alloc_version_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
		...
		case TP_CHIP_CFG_PROC_ITEM_ID: {
			if (1 == alloc_chip_cfg_proc_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_FLASH_TEST_ITEM_ID: {
			if (1 == alloc_flash_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_RECOVER_CFG_ITEM_ID: {
			if (1 == alloc_recover_cfg_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
		default:
...
s32 free_test_item_res(PST_TEST_ITEM *pp_test_item, u16 item_num)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		switch (p_test_item->test_item_id) {
			/*//----------------------tp open test-----------------------//*/
		case TP_RAWDATA_TEST_ITEMS_SET_ID: {
			if (1 == free_rawdata_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_DIFFDATA_TEST_ITEMS_SET_ID: {
			if (1 == free_diffdata_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
			...
			/*//----------------------hardware test-----------------------//*/
		case TP_SHORT_TEST_ITEM_ID: {
			if (1 == free_short_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
			...
			/*//----------------------firmware test-----------------------//*/
		case TP_VERSION_TEST_ITEM_ID: {
			if (1 == free_version_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
			/*//----------------------other test-----------------------//*/
		case TP_CHIP_CFG_PROC_ITEM_ID: {
			if (1 == free_chip_cfg_proc_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_FLASH_TEST_ITEM_ID: {
			if (1 == free_flash_test_item_res(p_test_item)) {
				ret = 1;
			}
		} break;

		case TP_RECOVER_CFG_ITEM_ID: {
			if (1 == free_recover_cfg_item_res(p_test_item)) {
				ret = 1;
			}
		} break;
		default:
...
s32 alloc_item_res_header(IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	p_item_res_header = (P_TEST_ITEM_RES)p_test_item->test_res.ptr_res;
	...
	if (NULL != p_test_item->p_sub_id_set) {
		if (alloc_mem_in_heap(
			...
			if (alloc_mem_in_heap(
				    ...
				    p_item_res_header->p_sub_id_set->len *
					    sizeof(u8))) {
				memcpy(p_item_res_header->p_sub_id_set->p_id_set,
				       p_test_item->p_sub_id_set->p_id_set,
				       p_test_item->p_sub_id_set->len);
			}
		}
		...
		p_item_res_header->test_item_err_code.len =
			p_test_item->p_sub_id_set->len +
			1; /*//each sub test for 1byte,maybe modify in future*/
		alloc_mem_in_heap(
			(void **)&(p_item_res_header->test_item_err_code
					   .ptr_err_code_set),
			p_item_res_header->test_item_err_code.len *
				sizeof(u16));
		memset(p_item_res_header->test_item_err_code.ptr_err_code_set,
		       0, p_item_res_header->test_item_err_code.len);
	} else {
		p_item_res_header->p_sub_id_set = NULL;
		p_item_res_header->test_item_err_code.len =
			1; /*//main test for 1byte,maybe modify in future*/
		alloc_mem_in_heap(
			(void **)&(p_item_res_header->test_item_err_code
					   .ptr_err_code_set),
			p_item_res_header->test_item_err_code.len *
		...
		memset(p_item_res_header->test_item_err_code.ptr_err_code_set,
		       0,
		       p_item_res_header->test_item_err_code.len * sizeof(u16));
	}
...
s32 free_item_res_header(IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	if (NULL != p_item_res_header->test_item_err_code.ptr_err_code_set) {
		free_mem_in_heap((void *)p_item_res_header->test_item_err_code
					 .ptr_err_code_set);
		p_item_res_header->test_item_err_code.ptr_err_code_set = NULL;
	}

	if (NULL != p_item_res_header->p_sub_id_set) {
		if (NULL != p_item_res_header->p_sub_id_set->p_id_set) {
			free_mem_in_heap((void *)p_item_res_header->p_sub_id_set
						 ->p_id_set);
			p_item_res_header->p_sub_id_set->p_id_set = NULL;
		}
		free_mem_in_heap((void *)p_item_res_header->p_sub_id_set);
		p_item_res_header->p_sub_id_set = NULL;
	}
...
s32 alloc_rawdata_test_item_res(IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	if (alloc_mem_in_heap((void **)pp_rawdata_res,
			      sizeof(ST_RAWDATA_TEST_RES))) {
		...
		if (alloc_mem_in_heap(
			...
			if (1 == alloc_mem_block(
				...
				(*pp_rawdata_res)->p_max_raw_tmp_data->len =
					p_tp_dev->sc_sen_num *
					p_tp_dev->sc_drv_num;
			}
		}
		...
		if (alloc_mem_in_heap(
			...
			if (alloc_mem_block(
				...
				(*pp_rawdata_res)->p_min_raw_tmp_data->len =
					p_tp_dev->sc_sen_num *
					p_tp_dev->sc_drv_num;
			}
		}
		...
		if (alloc_mem_in_heap(
			...
			if (alloc_mem_block(
				    ...
				    buf_size * data_byte)) {
				init_for_buffer(
					(u8 *)(p_cur_rawdata->p_data_buf),
					buf_size * data_byte);
			}
		}

		if (NULL != (*pp_rawdata_res)->p_max_raw_tmp_data->p_data_buf &&
		    NULL != (*pp_rawdata_res)->p_min_raw_tmp_data->p_data_buf &&
		    NULL != (*pp_rawdata_res)->p_beyond_rawdata_upper_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_rawdata_lower_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_jitter_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_accord_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_offset_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_key_upper_limit_cnt &&
		    NULL != (*pp_rawdata_res)->p_beyond_key_lower_limit_cnt &&
		    NULL != (*pp_rawdata_res)
				    ->item_res_header.test_item_err_code
				    .ptr_err_code_set &&
		    NULL != p_test_item->cur_data.ptr_cur_data &&
		    NULL != ((PST_CUR_RAWDATA)p_test_item->cur_data.ptr_cur_data)
				    ->p_data_buf) {
			ret = 1;
		}
	}
...
s32 free_rawdata_test_item_res(PST_TEST_ITEM p_test_item)
{
	...
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_rawdata_upper_limit_cnt);
	(*pp_rawdata_res)->p_beyond_rawdata_upper_limit_cnt = NULL;

	/*//free beyond rawdata lower limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_rawdata_lower_limit_cnt);
	(*pp_rawdata_res)->p_beyond_rawdata_lower_limit_cnt = NULL;

	/*//free beyond jitter limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_jitter_limit_cnt);
	(*pp_rawdata_res)->p_beyond_jitter_limit_cnt = NULL;

	/*//free beyond accord limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_accord_limit_cnt);
	(*pp_rawdata_res)->p_beyond_accord_limit_cnt = NULL;

	/*//free beyond offset limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_offset_limit_cnt);
	(*pp_rawdata_res)->p_beyond_offset_limit_cnt = NULL;

	/*//free beyond key upper limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_key_upper_limit_cnt);
	(*pp_rawdata_res)->p_beyond_key_upper_limit_cnt = NULL;

	/*//free beyond key lower limit buffer*/
	ret &= free_mem_in_heap(
		(void *)(*pp_rawdata_res)->p_beyond_key_lower_limit_cnt);
	(*pp_rawdata_res)->p_beyond_key_lower_limit_cnt = NULL;

	/*//free max tmp data buffer*/
	ret &= free_mem_block(
		(void *)(*pp_rawdata_res)->p_max_raw_tmp_data->p_data_buf);
	...
	ret &= free_mem_block(
		(void *)(*pp_rawdata_res)->p_min_raw_tmp_data->p_data_buf);
	...
	/*//free current data*/
	p_curdata = (PST_CUR_RAWDATA)p_test_item->cur_data.ptr_cur_data;
...
s32 alloc_diffdata_test_item_res(PST_TEST_ITEM p_test_item)
{
	...
	if (alloc_mem_in_heap((void **)pp_diffdata_res,
			      sizeof(ST_DIFFDATA_TEST_RES))) {
		ret = alloc_item_res_header(p_test_item);
		if (0 == ret) {
			return ret;
		}
		/*//max data tmp buffer*/
		if (alloc_mem_in_heap(
			    (void **)&((*pp_diffdata_res)->p_max_diff_tmp_data),
			    sizeof(ST_DIFF_TMP_DATA))) {
			buf_size = p_tp_dev->sc_drv_num * p_tp_dev->sc_sen_num;
			(*pp_diffdata_res)->p_max_diff_tmp_data->len = 0;
			if (1 ==
			    alloc_mem_block(
				    (void **)&((*pp_diffdata_res)
						       ->p_max_diff_tmp_data
						       ->p_data_buf),
				    buf_size * sizeof(s32))) {
				for (i = 0; i < buf_size; i++) {
...
s32 free_diffdata_test_item_res(PST_TEST_ITEM p_test_item)
{
	...
	/*//free max tmp data buffer*/
	ret &= free_mem_block(
		(void *)(*pp_diffdata_res)->p_max_diff_tmp_data->p_data_buf);
	...
	/*//free current data*/
	p_curdata = (PST_CUR_DIFFDATA)p_test_item->cur_data.ptr_cur_data;
	ret &= free_mem_block((void *)(p_curdata->p_data_buf));
	ret &= free_mem_in_heap_p(
		(void **)&(p_test_item->cur_data.ptr_cur_data));
...
s32 alloc_short_test_item_res(PST_TEST_ITEM p_test_item)
{
	...
	pp_test_res = (PST_SHORT_TEST_RES *)(&p_test_item->test_res.ptr_res);
	if (alloc_mem_in_heap((void **)pp_test_res,
			      sizeof(ST_SHORT_TEST_RES))) {
		ret = alloc_item_res_header(p_test_item);
...
s32 alloc_version_test_item_res(PST_TEST_ITEM p_test_item)
{
	...
	pp_test_res = (PST_VERSION_TEST_RES *)(&p_test_item->test_res.ptr_res);
	if (alloc_mem_in_heap((void **)pp_test_res,
			      sizeof(ST_VERSION_TEST_RES))) {
		ret = alloc_item_res_header(p_test_item);
...
s32 alloc_chip_cfg_proc_item_res(PST_TEST_ITEM p_test_item)
{
	...
	if (alloc_mem_in_heap((void **)pp_test_res,
			      sizeof(ST_CHIP_CONFIG_PROC_RES))) {
		ret = alloc_item_res_header(p_test_item);
		p_param = (PST_CHIP_CONFIG_PROC_PARAM)
				  p_test_item->param.ptr_param;
		if (NULL != p_param && 0 < p_param->cfg_len) {
			(*pp_test_res)->cfg_len = p_param->cfg_len;
			ret = alloc_mem_in_heap(
				(void **)(&(*pp_test_res)->p_read_cfg),
				(*pp_test_res)->cfg_len * sizeof(u8));
		} else {
...
extern s32 alloc_recover_cfg_item_res(PST_TEST_ITEM p_test_item)
{
	...
	if (alloc_mem_in_heap((void **)pp_test_res,
			      sizeof(ST_RECOVER_CHIP_CFG_RES))) {
		ret = alloc_item_res_header(p_test_item);
		if (NULL != p_param) {
			(*pp_test_res)->cfg_len = p_param->cfg_len;
			ret = alloc_mem_in_heap(
