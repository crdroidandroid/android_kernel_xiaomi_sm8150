...
* Return		: u8(0:have not handled 1:handled)
*******************************************************************************/
extern s32 board_read_chip_reg(PST_TP_DEV p_dev, u16 addr, u8 *p_buf,
			       u16 buf_len)
{
...
* Return		: u8(0:have not handled 1:handled)
*******************************************************************************/
extern s32 board_write_chip_reg(PST_TP_DEV p_dev, u16 addr, u8 *p_buf,
				u16 buf_len)
{
...
extern s32 board_enter_rawdata_mode(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_write(p_dev, p_dev->cmd_set.rawdata_cmd.addr,
...
extern s32 board_enter_coord_mode(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_write(p_dev, p_dev->cmd_set.coorddata_cmd.addr,
				     p_dev->cmd_set.coorddata_cmd.cmd_buf,
				     p_dev->cmd_set.coorddata_cmd.cmd_len);
		if (1 == ret) {
			break;
		}
	}
	...
	if ((p_dev->chip_type == TP_NORMANDY) ||
		...
		if (ret == 1) {
			if ((p_dev->rawdata_option & _DATA_LARGE_ENDIAN) ||
			    (p_dev->rawdata_option & _DATA_DRV_SEN_INVERT)) {
				reshape_data(p_dev->rawdata_option, p_dev,
					     p_rawdata);
			}
		}
		return ret;
	} else {
		return get_rawdata(p_dev, time_out_ms, b_filter_data,
				   p_rawdata);
	}
}
	...
	if ((p_dev->chip_type == TP_NORMANDY) ||
		...
		for (i = 0; i < retry_cnt; i++) {
			ret = sync_read_rawdata(
				p_dev->diffdata_addr, p_diffdata->p_data_buf,
				p_dev->diffdata_len * data_size);
			if (ret > 0) {
				ret = 1;
				break;
			}
		...
		if (ret == 1) {
			if ((p_dev->diffdata_option & _DATA_LARGE_ENDIAN) ||
			    (p_dev->diffdata_option & _DATA_DRV_SEN_INVERT)) {
				reshape_data(p_dev->diffdata_option, p_dev,
					     p_diffdata);
			}
		}
		return ret;
	} else {
		return get_diffdata(p_dev, time_out_ms, b_filter_data,
				    p_diffdata);
	}
}
...
extern s32 board_chip_hid_opr(PST_TP_DEV p_dev, s32 b_ena)
{
	...
	if (1 == b_ena) {
		for (i = 0; i < retry_cnt; i++) {
			ret = board_write_chip_reg(
				p_dev, p_dev->cmd_set.hid_ena_cmd.addr,
				p_dev->cmd_set.hid_ena_cmd.cmd_buf,
				p_dev->cmd_set.hid_ena_cmd.cmd_len);
			/*
			cmd_buf[0] = 0xbb;
			cmd_buf[1] = 0x00;
			cmd_buf[2] = 0x45;
		...
		for (i = 0; i < retry_cnt; i++) {
			ret = board_write_chip_reg(
				p_dev, p_dev->cmd_set.hid_dis_cmd.addr,
				p_dev->cmd_set.hid_dis_cmd.cmd_buf,
				p_dev->cmd_set.hid_dis_cmd.cmd_len);
			/*
			cmd_buf[0] = 0xaa;
			cmd_buf[1] = 0x00;
			cmd_buf[2] = 0x56;
...
extern s32 board_set_power_volt(PST_TP_DEV p_dev, u16 volt, u16 aver_len,
				u8 cnt)
{
	return 1;
}
...
extern void board_gpio_mode_set(PST_TP_DEV p_dev, u8 gpio_type, u16 gpio_pin,
				u8 gpio_mode)
{
	;
}
