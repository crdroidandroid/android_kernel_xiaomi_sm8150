...
extern s32 tp_chip_cfg_proc(IN_OUT ptr32 p_data)
{
	...
	if (p_param->option == CHIP_CFG_SEND) {
		/*write cfg*/
		ret &= force_update_cfg(p_dev, p_param->config,
					p_param->cfg_len);
		if (0 == ret) {
	...
	/*read cfg*/
	if (read_cfg_arr(p_dev, p_test_res->p_read_cfg, p_test_res->cfg_len) ==
	    0) {
		ret = 0;
	} else {
		ret &= 1;
	...
	if (0 == ret) {
		...
		board_print_error("[send config test]read config error!\n");
		goto SEN_CFG_END;
	}
...
extern s32 tp_send_spec_cfg_to_chip(IN_OUT ptr32 p_data)
{
	...
	p_test_res = (PST_SEND_SPEC_CONFIG_RES)p_test_item->test_res.ptr_res;
	p_dev = p_test_item->p_tp_dev;
	...
	if (read_cfg_arr(p_dev, p_test_res->read_config,
	...
	if (0 == ret) {
		...
		p_test_res->item_res_header.test_result = TEST_NG;
		p_test_res->item_res_header.test_status = TEST_ABORTED;
		board_print_error("[send special config]read config error!\n");
		goto SEN_SPC_CFG_END;
	}
	/*check cfg*/
...
extern s32 tp_flash_test(IN_OUT ptr32 p_data)
{
	...
	if ((p_param->option & FLASH_TEST_SEND_CFG_OPT) ==
		...
		/*write cfg*/
		ret &= force_update_cfg(p_dev, p_param->config_arr,
					p_param->cfg_len);
		if (0 == ret) {
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[0] =
			...
			p_test_res->item_res_header.test_result = TEST_NG;
			board_print_error("[flash test]update config error!\n");
			goto FLASH_TEST_END;
		}
	}
	...
	/*read cfg*/
	if (read_cfg_arr(p_dev, p_test_res->config_arr, p_test_res->cfg_len) ==
	    0) {
		ret = 0;
	} else {
		ret &= 1;
...
extern s32 tp_recover_cfg_to_chip(IN_OUT ptr32 p_data)
{
	...
	if (0 == ret) {
		...
		p_test_res->item_res_header.test_result = TEST_NG;
		p_test_res->item_res_header.test_status = TEST_ABORTED;
		board_print_error("[recover config test]read config error!\n");
		goto RECOVER_CFG_END;
	}
	/*check cfg*/
