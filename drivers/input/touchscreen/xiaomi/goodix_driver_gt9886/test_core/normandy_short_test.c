...
extern s32 normandy_short_test(IN_OUT ptr32 p_data)
{
	...
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code
			.ptr_err_code_set[0] = SHORT_TEST_SEND_PARAMETER_FAIL;
		p_test_res->item_res_header.test_status = TEST_ABORTED;
		p_test_res->item_res_header.test_result = TEST_NG;
		board_print_error("%s\n",
	...
	if (0 == ret) {
		p_test_res->item_res_header.test_item_err_code
			.ptr_err_code_set[0] = SHORT_TEST_ANALYSIS_RESULT_FAIL;
		p_test_res->item_res_header.test_status = TEST_ABORTED;
		p_test_res->item_res_header.test_result = TEST_NG;
		board_print_error(
			"%s\n",
...
static s32 send_short_param(IN ptr32 p_data)
{
	...
	*/
	buf_tmp[0] = ((p_short_param->short_threshold.gt_short_threshold >> 8) &
		      0xFF);
	buf_tmp[1] =
...
static s32 wait_short_end(IN PST_TP_DEV p_dev)
{
	...
	board_delay_ms(1800);
	/*wait test end*/
	ret = wait_status(p_dev, NORM_SHORT_TESTEND_ADDR,
			  NORM_SHORT_ALL_TEST_END, 1200);
	if (0 == ret) {
		board_print_error("%s\n", "wait test end fail!");
		goto WAIT_END;
	}
			  ...
			  IN u16 self_code, IN u16 short_code)
{
	...
	if (!b_in_same_grp) {
		...
		die_num1 = (die_num1 >= die_num2) ? (die_num1 - die_num2) :
						    (die_num2 - die_num1);
...
static s32 analyse_short_res(IN_OUT ptr32 p_data)
{
	...
	/*judge short flag*/
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_read(p_dev, NORM_SHORT_TEST_RESULT_ADDR,
				    &short_flag, 1);
		if (1 == ret) {
			break;
		}
	...
	*/
	...
	if (0x08 == (short_flag & 0x08)) {
		...
		for (i = 0; i < retry_cnt; i++) {
			...
			ret = chip_reg_read(p_dev, NORM_SHORT_DIFFCODE_ADDR,
		...
		for (i = 0; i < data_len_tmp; i += 2) {
			diff_code_tmp = (p_tmp_data_arr[i] << 8) +
					p_tmp_data_arr[i + 1];
			/*calculate short_r*/
			short_r = calc_short_gnd_vdd(
				diff_code_tmp, (10 * p_dev->power_volt / 1000),
				&chn_id_tmp2);
			chn_id_tmp1 = i >> 1;
	...
	for (i = 0; i < data_len_tmp / 2; i++) {
		p_self_code_arr[i] = ((p_tmp_data_arr[2 * i] << 8) +
				      p_tmp_data_arr[2 * i + 1]) &
				     0x7FFF;
	}
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		*/
		ret = chip_reg_read(p_dev, NORM_SHORT_NUMBER_ADDR,
				    chn_short_cnt, 3);
		if (1 == ret) {
			break;
		}
				...
				for (j = 0; j < max_drv_num_in_die; j++) {
					short_code_tmp =
						...
						p_tmp_data_arr[NORM_SHORT_HEAD +
							       j * 2 + 1];
					chn_id_tmp1 |= CHANNEL_TX_FLAG;
					chn_id_tmp2 = (j + max_sen_num_in_die) |
						      CHANNEL_TX_FLAG;
					if (short_code_tmp >=
					    p_short_param->short_threshold
						    .gt_short_threshold) {
						id_tmp = short_id_to_selfdata_id(
							p_dev, chn_id_tmp1);
						short_r = calc_short_chn(
							p_dev, chn_id_tmp1,
							chn_id_tmp2,
							p_self_code_arr[id_tmp],
							short_code_tmp);
						if (short_r <
						    p_short_param->short_threshold
	...
	if (0x02 == (short_flag & 0x02)) {
		...
		*/
		...
		for (i = 0; i < chn_short_cnt[1]; i++) {
			...
			/*calc resistor*/
			chn_id_tmp1 = (p_tmp_data_arr[0] << 8) +
				      p_tmp_data_arr[1]; /*Rx*/
			{
				/*Rx*/
