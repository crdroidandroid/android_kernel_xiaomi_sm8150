...
extern s32 fw_version_test(IN_OUT ptr32 p_data)
{
	...
	p_test_res = (PST_VERSION_TEST_RES)p_test_item->test_res.ptr_res;
	...
	if (p_test_res->item_res_header.test_result == TEST_OK) {
		...
		if (0 == p_ver_param->b_auto_update) {
			...
			if (0 == ret) {
				...
				err_code = p_test_res->item_res_header
						   .test_item_err_code
						   .ptr_err_code_set[0];
				goto TEST_ITEM_END;
			}
...
static s32 compare_version(PST_TEST_ITEM p_test_item)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		ret = chip_reg_read(p_test_item->p_tp_dev,
	...
	} else if (p_ver_param->ver_cmp_opt == VERSION_CMP_GREATOR_EQUAL) {
	...
	if (0 == cmp_ret) {
		...
		p_test_res->item_res_header.test_item_err_code
			.ptr_err_code_set[0] = VERSION_TEST_VERSION_MATCH_FAIL;
		p_test_res->item_res_header.test_result = TEST_NG;
		ret = 0;
		goto COMPARE_END;
	} else {
