...
extern s32 short_test_func(IN_OUT ptr32 p_data)
{
	...
	p_test_res->item_res_header.test_item_err_code.ptr_err_code_set[0] =
		SHORT_TEST_NONE_ERROR;
	...
	if (0 == ret) {
		board_print_error("[short test]disable device status faild!\n");
	...
	err_code = p_test_res->item_res_header.test_item_err_code
			   .ptr_err_code_set[0];
...
SHORTTEST_END:
	...
	if (ret == 0) {
		board_print_warning("[short test]recovry chip status error!\n");
...
extern s32 clear_short_mess_in_res(IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	p_short_res->item_res_header.test_id = p_test_item->test_item_id;
	p_short_res->item_res_header.test_status = TEST_START;
	p_short_res->item_res_header.test_result = TEST_OK;
	p_short_res->item_res_header.test_item_err_code.ptr_err_code_set[0] =
		SHORT_TEST_NONE_ERROR;
	return 1;
}
				   ...
				   OUT PST_SHORT_TEST_RES p_short_res)
{
	...
	if (repeat == 0) {
		p_short_res->short_mess_arr[4 * p_short_res->short_num + 0] =
			chn1;
		p_short_res->short_mess_arr[4 * p_short_res->short_num + 1] =
			chn2;
		p_short_res->short_mess_arr[4 * p_short_res->short_num + 2] =
			(r >> 8) & 0xFF;
		p_short_res->short_mess_arr[4 * p_short_res->short_num + 3] =
			r & 0xFF;
				   ...
				   IN ENUM_CHN_MAP_MODE chn_enum)
{
	...
	for (i = 0; i < p_short_res->short_num * 4; i++) {
		...
		case TX_RX_SHARED: {
			b_find = 0;
			j = p_short_res->short_mess_arr[i] & (~CHANNEL_TX_FLAG);

			if (0 == b_find) {
				for (ch_cnt = 0; ch_cnt < p_dev->max_drv_num;
				     ch_cnt++) {
					if (p_dev->drv_map[ch_cnt] == j) {
						p_short_res->short_mess_arr[i] =
							CHANNEL_TX_FLAG |
							ch_cnt;
						valid_chn_mark[i] = 1;
						b_find = 1;
			...
			if (0 == b_find) {
				for (ch_cnt = 0; ch_cnt < p_dev->max_sen_num;
				     ch_cnt++) {
					if (p_dev->sen_map[ch_cnt] == j) {
						p_short_res->short_mess_arr[i] =
							ch_cnt;
						valid_chn_mark[i] = 1;
						b_find = 1;
		...
		case TX_RX_PACK_ADC: {
			...
			if ((j & CHANNEL_TX_FLAG) ==
			    CHANNEL_TX_FLAG) { /*//Drv Chancel*/
				...
				for (ch_cnt = 0; ch_cnt < p_dev->max_drv_num;
					...
					if (p_dev->drv_map[ch_cnt] == j) {
						ic_chn_num =
							p_dev->pack_drv_2_adc_map
								[ch_cnt];
						if (ch_cnt <
						    (p_dev->max_drv_num /
						     2)) { /*//the first G1 chancel*/
							...
							for (k = 0;
							     ...
							     k++) {
								if (p_dev->cfg
									    [p_dev->drv_start_addr -
									     p_dev->cfg_start_addr +
									     k] ==
								    ic_chn_num) {
									p_short_res
										->short_mess_arr
		...
		default: {
			if ((p_short_res->short_mess_arr[i] &
			     CHANNEL_TX_FLAG) == CHANNEL_TX_FLAG) {
				j = p_short_res->short_mess_arr[i] &
				    (~CHANNEL_TX_FLAG);
				for (ch_cnt = 0; ch_cnt < p_dev->max_drv_num;
				     ch_cnt++) {
	...
	for (i = 0; i < p_short_res->short_num * 4; i += 4) {
		if ((valid_chn_mark[i] != 1 &&
			...
			if (p_short_res->short_num * 4 - i - 4 >= 4) {
				memcpy(p_short_mess_tmp, p_short_mess_tmp + 4,
				       p_short_res->short_num * 4 - (i + 4));
			}
		} else {
			ic_chn_num++;
			p_short_mess_tmp += 4;
