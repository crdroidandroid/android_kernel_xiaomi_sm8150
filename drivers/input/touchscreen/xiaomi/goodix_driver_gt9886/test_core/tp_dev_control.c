...
*******************************************************************************/
extern s32 chip_reg_write(IN PST_TP_DEV p_dev, IN u16 addr, IN u8 *p_buf,
			  IN u16 buf_len)
{
	s32 ret = 0;
	u8 retry_cnt = 3;
	...
	case TP_NORMANDY:
	case TP_OSLO: {
		ret = normandy_reg_write(p_dev, addr, p_buf, buf_len);
	} break;
	default: {
...
* Return		: s32(0:Fail 1:Ok)
*******************************************************************************/
extern s32 chip_reg_read(IN PST_TP_DEV p_dev, IN u16 addr, OUT u8 *p_buf,
			 IN u16 buf_len)
{
	s32 ret = 0;
	u8 retry_cnt = 3;
...
* Return		: s32(0:Fail 1:Ok)
*******************************************************************************/
extern s32 chip_reg_write_chk(IN PST_TP_DEV p_dev, IN u16 addr, IN u8 *p_buf,
			      IN u16 buf_len)
{
	s32 ret = 0;
	u16 i = 0;
...
extern s32 get_data_flag(IN PST_TP_DEV p_dev)
{
	...
	for (i = 0; i < retry_cnt; i++) {
		...
		if ((1 == ret) && (0 != (p_dev->syncflag_mask & data_temp))) {
			ret = 1;
			break;
		} else {
			ret = 0;
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 force_update_cfg(IN PST_TP_DEV p_dev, IN u8 *config, IN u16 cfg_len)
{
	s32 ret = 0;
	switch (p_dev->chip_type) {
	case TP_NORMANDY:
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 write_cmd(IN PST_TP_DEV p_dev, IN u16 cmd_addr, IN u8 cmd_byte)
{
	s32 ret = 0;
	switch (p_dev->chip_type) {
	case TP_GT900:
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 write_cfg_arr(IN PST_TP_DEV p_dev, IN u8 *config, IN u16 cfg_len)
{
	s32 ret = 0;
	switch (p_dev->chip_type) {
	case TP_NORMANDY:
...
* Return		: s32(0:fail >0:read config len)
*******************************************************************************/
extern s32 read_cfg_arr(IN PST_TP_DEV p_dev, IN_OUT u8 *config, IN u16 cfg_len)
{
	s32 ret = 0;
	switch (p_dev->chip_type) {
	case TP_NORMANDY:
