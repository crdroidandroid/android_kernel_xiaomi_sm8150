...
extern s32 rawdata_test_sets_func(IN_OUT ptr32 p_data)
{
	...
	if (NULL != p_test_item->ptr_test_item_before_func) {
		ret = p_test_item->ptr_test_item_before_func(
			p_test_item->ptr_test_item_before_func_param);
		if (ret == 0) {
			board_print_error(
	...
	p_test_proc_data = (PST_TEST_PROC_DATA)p_test_item->ptr_test_proc_data;
	...
	ret = chip_mode_select(p_test_item->p_tp_dev, CHIP_RAW_DATA_MODE);
	if (0 == ret) {
	...
	p_cur_data = (PST_CUR_RAWDATA)p_test_item->cur_data.ptr_cur_data;
	p_cur_data->cur_frame_id = 0;
	while (1) {
		if (p_test_proc_data->test_break_flag) {
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[0] = RAWDATA_TEST_BREAK_OUT;
			p_test_res->item_res_header.test_status = TEST_ABORTED;
		...
		ret = board_get_rawdata(p_test_item->p_tp_dev,
					get_rawdata_timeout, b_filter_data,
					p_cur_data);
		if (0 == ret) {
		...
		p_spec_callback_set =
			(PST_RAWDATA_TEST_CALLBACK_SET)
				p_test_item->ptr_special_callback_data;
		if (NULL != p_spec_callback_set) {
			/*save current data*/
			ret = p_spec_callback_set->ptr_save_cur_data_func(
				p_spec_callback_set
		...
		if ((cur_frame_total_num <= max_frame_total_num) &&
		    (0 == p_cur_data->cur_frame_id % add_frame_num)) {
			ret = analyze_rawtest_res(offse_and_accord_flag,
						  p_test_item);
			if (ret == 0 || ret == 1) {
				break;
			} else if (ret == 2) {
...
EXIT_RAWDATA_TEST:
	...
	if (NULL != p_test_item->ptr_test_item_finished_func) {
		ret = p_test_item->ptr_test_item_finished_func(
			p_test_item->ptr_test_item_finished_func_param);
		if (ret == 0) {
			board_print_error(
...
extern s32 rawdata_max_test_func(IN_OUT ptr32 p_data)
{
	...
	if (((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0) &&
	    ((p_raw_test_res->open_test_result & BEYOND_RAWDATA_UPPER_LIMIT) ==
	     0)) {
		return ret;
	}
	...
	if (0xFF != index) {
		...
		for (id = 0; id < node_num; id++) {
			if (get_chn_need_check(p_test_item, id)) {
				...
				para_tmp = get_chn_upper_limit(p_test_item, id);
...
extern s32 rawdata_min_test_func(IN_OUT ptr32 p_data)
{
	...
	/*judge test item result*/
	if (((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0) &&
	    ((p_raw_test_res->open_test_result & BEYOND_RAWDATA_LOWER_LIMIT) ==
	     0)) {
		return ret;
	}
...
extern s32 rawdata_accord_test_func(IN_OUT ptr32 p_data)
{
	...
	p_raw_test_res = (PST_RAWDATA_TEST_RES)p_test_item->test_res.ptr_res;
	if ((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0 &&
	    (p_raw_test_res->open_test_result & BEYOND_ACCORD_LIMIT) == 0) {
		return ret;
	}
	...
	if (0xFF != index) {
		...
		for (id = 0; id < node_num; id++) {
			if (get_chn_need_check(p_test_item, id)) {
				...
				/*/upper limit*/
				para_tmp =
					get_chn_accord_limit(p_test_item, id);
				if (data_tmp > para_tmp) {
					/*mark this channel beyond accord limit count*/
					p_raw_test_res
						->p_beyond_accord_limit_cnt[id]++;
					p_raw_test_res->open_test_result |=
						BEYOND_ACCORD_LIMIT;
					p_raw_test_res->item_res_header
						.test_item_err_code
...
extern s32 keydata_max_test_func(IN_OUT ptr32 p_data)
{
	...
	/*judge test item result*/
	if (((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0) &&
	    ((p_raw_test_res->open_test_result & KEY_BEYOND_UPPER_LIMIT) ==
	     0)) {
		return ret;
	}
...
extern s32 keydata_min_test_func(IN_OUT ptr32 p_data)
{
	...
	/*judge test item result*/
	if (((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0) &&
	    ((p_raw_test_res->open_test_result & KEY_BEYOND_LOWER_LIMIT) ==
	     0)) {
		return ret;
	}
...
extern s32 rawdata_jitter_test_func(IN_OUT ptr32 p_data)
{
	...
	if (0xFF != index) {
		...
		for (id = 0; id < node_num; id++) {
			if (get_chn_need_check(p_test_item, id)) {
				data_tmp = p_raw_test_res->p_max_raw_tmp_data
						   ->p_data_buf[id] -
...
extern s32 rawdata_uniform_test_func(IN_OUT ptr32 p_data)
{
	...
	p_raw_test_res = (PST_RAWDATA_TEST_RES)p_test_item->test_res.ptr_res;
	if ((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0 &&
	    (p_raw_test_res->open_test_result & BEYOND_UNIFORMITY_LIMIT) == 0) {
		return ret;
	}
...
extern s32 rawdata_offset_test_func(IN_OUT ptr32 p_data)
{
	...
	p_raw_test_res = (PST_RAWDATA_TEST_RES)p_test_item->test_res.ptr_res;
	if ((p_raw_test_res->open_test_result & FIRST_TEST_FLAG) != 0 &&
	    (p_raw_test_res->open_test_result & BEYOND_OFFSET_LIMIT) == 0) {
		return ret;
	}

	index = get_arr_id_from_sub_item(p_test_item, TP_OFFSET_TEST_ITEM_ID);
	if (0xFF != index) {
		u16 id = 0;
		u16 node_num = 0;
		s32 data_tmp = 0;
...
extern s32 diffdata_test_sets_func(IN_OUT ptr32 p_data)
{
	...
	p_cur_data = (PST_CUR_DIFFDATA)p_test_item->cur_data.ptr_cur_data;
	...
	ret = chip_mode_select(p_test_item->p_tp_dev, CHIP_DIFF_DATA_MODE);
	if (0 == ret) {
	...
	while (1) {
		if (p_test_proc_data->test_break_flag) {
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[0] = DIFFDATA_TEST_BREAK_OUT;
			p_test_res->item_res_header.test_result = TEST_NG;
			p_test_res->item_res_header.test_status = TEST_ABORTED;
			goto DIFFDATA_TEST_SETS_FUNC_END;
		}
		...
		ret = board_get_diffdata(p_test_item->p_tp_dev,
					 get_diffdata_timeout, b_filter_data,
					 p_cur_data);
		if (0 == ret) {
		...
		p_spec_callback_set =
			(PST_DIFFDATA_TEST_CALLBACK_SET)
				p_test_item->ptr_special_callback_data;
		if (NULL != p_spec_callback_set) {
			/*//save current data*/
			p_spec_callback_set->ptr_save_cur_data_func(
				p_spec_callback_set
					->ptr_save_cur_data_func_param);
		}
		/*//count frame id*/
		...
		if ((cur_frame_total_num <= max_frame_total_num) &&
		    (0 == p_cur_data->cur_frame_id % add_frame_num)) {
			ret = analyze_difftest_res(p_cur_data->cur_frame_id,
						   p_test_item);
			if (ret == 0) {
				break;
			} else if (ret == 1) {
...
DIFFDATA_TEST_SETS_FUNC_END:
	...
	if (NULL != p_test_item->ptr_test_item_finished_func) {
		if (0 ==
		    p_test_item->ptr_test_item_finished_func(
			    p_test_item->ptr_test_item_finished_func_param)) {
			board_print_error(
...
extern s32 get_rawdata(IN PST_TP_DEV p_dev, IN u16 time_out_ms,
		       IN u8 b_filter_data, OUT PST_CUR_RAWDATA p_cur_data)
{
	s32 ret = 0;
	u16 i = 0;
	u8 retry_cnt = 3;
		...
		if (data_flag) {
			if (0 == b_filter_data) {
				for (i = 0; i < retry_cnt; i++) {
					ret = chip_reg_read(
						p_dev, p_dev->rawdata_addr,
						p_cur_data->p_data_buf,
			...
			if (0 == ret) {
				board_print_error("clear dataflag error!\n");
				goto GET_RAWDATA_END;
			}
			goto GET_RAWDATA_END;
		}
...
extern s32 get_diffdata(IN PST_TP_DEV p_dev, IN u16 time_out_ms,
			IN u8 b_filter_data, IN_OUT PST_CUR_DIFFDATA p_cur_data)
{
	s32 ret = 0;
	u8 i = 0;
	u8 retry_cnt = 3;
	...
	while (wait_time < time_out_ms) {
		...
		if (get_data_flag(p_dev)) {
			if (0 == b_filter_data) {
				for (i = 0; i < retry_cnt; i++) {
					ret = chip_reg_read(
						p_dev, p_dev->diffdata_addr,
						(u8 *)p_cur_data->p_data_buf,
						p_dev->diffdata_len *
							data_size);
					if (1 == ret) {
						break;
			 ...
			 IN_OUT PST_CUR_RAWDATA p_cur_data)
{
	...
	if ((data_option & _DATA_LARGE_ENDIAN) && (data_size > 1)) {
		for (i = 0; i < p_dev->rawdata_len * data_size;
			...
			for (j = 0; j < data_size - j; j++) {
				...
				p_buf_tmp[i + data_size - j - 1] = data_tmp;
			}
		}
	}
	...
	if ((data_option & _DATA_DRV_SEN_INVERT)) {
		if (alloc_mem_block((void **)&p_buf_bak,
				    p_dev->rawdata_len * data_size)) {
			memcpy(p_buf_bak, p_buf_tmp,
			       p_dev->rawdata_len * data_size);
			for (i = 0; i < p_dev->sc_drv_num * p_dev->sc_sen_num;
			     i++) {
				drv_id = i / p_dev->sc_sen_num;
				sen_id = i % p_dev->sc_sen_num;
				tmp_id = sen_id * p_dev->sc_drv_num + drv_id;
				for (j = 0; j < data_size; j++) {
					p_buf_tmp[tmp_id * data_size + j] =
						p_buf_bak[i * data_size + j];
				}
			}
...
extern s32 get_chn_rawdata(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	...
	if (len > chn_id) {
		if (p_dev->rawdata_option & _DATA_BIT_8) {
			...
			ret = (s16)((p_cur_rawdata->p_data_buf[chn_id * 2]) +
				    (p_cur_rawdata->p_data_buf[chn_id * 2 + 1]
				     << 8));
		} else if (p_dev->rawdata_option & _DATA_BIT_32) {
			ret = (p_cur_rawdata->p_data_buf[chn_id * 4]) +
			      (p_cur_rawdata->p_data_buf[chn_id * 4 + 1] << 8) +
			      (p_cur_rawdata->p_data_buf[chn_id * 4 + 2]
			       << 16) +
			      (p_cur_rawdata->p_data_buf[chn_id * 4 + 3] << 24);
		}
	}
	return ret;
}
...
*******************************************************************************/
extern s32 get_chn_need_check(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	s32 ret = 0;
...
* Return		: s32
*******************************************************************************/
extern s32 get_chn_upper_limit(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	s32 ret = 0;
	u16 i = 0;
...
* Return		: s32
*******************************************************************************/
extern s32 get_chn_lower_limit(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	s32 ret = 0;
	u16 i = 0;
	PST_RAWDATA_TEST_PARAM p_rawdata_test_param =
...
extern s32 get_chn_accord(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	...
	for (i = 0; i < 4; i++) {
		...
		tmp_data = (tmp_data - cur_data) > 0 ? (tmp_data - cur_data) :
						       (cur_data - tmp_data);
...
* Return		: s32
*******************************************************************************/
extern s32 get_chn_accord_limit(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	s32 ret = 0;
	u16 i = 0;
	PST_RAWDATA_TEST_PARAM p_rawdata_test_param =
...
*******************************************************************************/
extern u8 get_adjacent_node(IN u16 node_id, IN PST_TP_DEV p_dev, OUT s16 *p_arr)
{
	u8 ret = 4;
	u16 drv_id = 0; /*//col_id*/
	u16 sen_id = 0; /*//row_id*/
	...
	p_arr[ADJ_POS_DOWN] = (sen_id + 1) * (p_dev->sc_drv_num) + drv_id;
	p_arr[ADJ_POS_LEFT] = (sen_id) * (p_dev->sc_drv_num) + drv_id - 1;
	p_arr[ADJ_POS_RIGHT] = (sen_id) * (p_dev->sc_drv_num) + drv_id + 1;
...
extern s32 get_chn_offset(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	...
	if (len > chn_id) {
		...
		tmp_data = (tmp_raw - avg) > 0 ? (tmp_raw - avg) :
						 (avg - tmp_raw);
...
* Return		: s32
*******************************************************************************/
extern s32 get_chn_offset_limit(IN PST_TEST_ITEM p_test_item, IN u16 chn_id)
{
	s32 ret = 0;
	u16 i = 0;
	PST_RAWDATA_TEST_PARAM p_rawdata_test_param =
...
extern s32 get_keydata(IN PST_TEST_ITEM p_test_item, IN u8 key_id)
{
	...
	if (key_id < p_dev->key_num) {
		...
		if (p_dev->rawdata_option & _DATA_BIT_8) {
			...
			ret = (s16)((p_cur_rawdata->p_data_buf[tmp_id * 2]) +
				    (p_cur_rawdata->p_data_buf[tmp_id * 2 + 1]
				     << 8));
		} else if (p_dev->rawdata_option & _DATA_BIT_32) {
			ret = (p_cur_rawdata->p_data_buf[tmp_id * 4]) +
			      (p_cur_rawdata->p_data_buf[tmp_id * 4 + 1] << 8) +
			      (p_cur_rawdata->p_data_buf[tmp_id * 4 + 2]
			       << 16) +
			      (p_cur_rawdata->p_data_buf[tmp_id * 4 + 3] << 24);
		}
	}
	return ret;
}
...
* Return		: s32(0:not need check 1:need check)
*******************************************************************************/
extern s32 get_key_need_check(IN PST_TEST_ITEM p_test_item, IN u16 key_id)
{
	s32 ret = 0;
	u16 key_pos = 0;
	...
	if (key_id < p_dev->key_num) {
		...
		if (p_rawdata_test_param->p_raw_key_limit->p_need_check_key
			    ->p_data_buf[key_pos] == TP_FLAG_KEY_NEED_CHECK) {
			ret = 1;
		}
	}
	return ret;
...
* Return		: s32
*******************************************************************************/
extern s32 get_key_upper_limit(IN PST_TEST_ITEM p_test_item, IN u8 key_id)
{
	s32 ret = 0;
	PST_RAWDATA_TEST_PARAM p_rawdata_test_param =
		(PST_RAWDATA_TEST_PARAM)(p_test_item->param.ptr_param);
	...
	for (i = 0; i < p_spec_node->data_len; i++) {
		...
		if ((node_id >= p_tp_dev->sc_drv_num * p_tp_dev->sc_sen_num) &&
		    node_id - p_tp_dev->sc_drv_num * p_tp_dev->sc_sen_num ==
			    key_id) {
			ret = p_spec_node->p_data_buf[i].high_raw_limit;
		}
	}
...
* Return		: s32
*******************************************************************************/
extern s32 get_key_lower_limit(IN PST_TEST_ITEM p_test_item, IN u8 key_id)
{
	s16 ret = 0;
	PST_RAWDATA_TEST_PARAM p_rawdata_test_param =
		(PST_RAWDATA_TEST_PARAM)(p_test_item->param.ptr_param);
...
extern s32 get_rawdata_jitter_value(IN PST_TEST_ITEM p_test_item)
{
	...
	for (i = 0; i < p_raw_test_res->p_max_raw_tmp_data->len; i++) {
		tmp_data = p_raw_test_res->p_max_raw_tmp_data->p_data_buf[i] -
...
extern s32 get_chn_diffdata(PST_TEST_ITEM p_test_item, u16 chn_id)
{
	...
	if (len > chn_id) {
		if (p_dev->diffdata_option & _DATA_BIT_8) {
			...
			ret = (s16)((p_cur_diffdata->p_data_buf[chn_id * 2]) +
				    (p_cur_diffdata->p_data_buf[chn_id * 2 + 1]
				     << 8));
		} else if (p_dev->diffdata_option & _DATA_BIT_32) {
			ret = (p_cur_diffdata->p_data_buf[chn_id * 4]) +
			      (p_cur_diffdata->p_data_buf[chn_id * 4 + 1]
			       << 8) +
			      (p_cur_diffdata->p_data_buf[chn_id * 4 + 2]
			       << 16) +
			      (p_cur_diffdata->p_data_buf[chn_id * 4 + 3]
			       << 24);
		}
	}
	return ret;
...
static s32 clear_rawtest_res_buf(IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	p_test_res->item_res_header.test_status = TEST_START;
	p_test_res->item_res_header.test_result = TEST_NG;
	for (i = 0; i < p_test_res->item_res_header.test_item_err_code.len;
	     i++) {
		p_test_res->item_res_header.test_item_err_code
			       ...
			       IN_OUT PST_TEST_ITEM p_test_item)
{
	...
	if ((p_test_res->open_test_result & BEYOND_RAWDATA_UPPER_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, BEYOND_RAWDATA_UPPER_LIMIT,
			p_test_res->p_beyond_rawdata_upper_limit_cnt, node_num);

		if (0 == (test_res_unstb & BEYOND_RAWDATA_UPPER_LIMIT) &&
		    (p_test_res->open_test_result &
		     BEYOND_RAWDATA_UPPER_LIMIT) != 0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_RAWDATA_MAX_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
	...
	if ((p_test_res->open_test_result & BEYOND_RAWDATA_LOWER_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, BEYOND_RAWDATA_LOWER_LIMIT,
			p_test_res->p_beyond_rawdata_lower_limit_cnt, node_num);

		if (0 == (test_res_unstb & BEYOND_RAWDATA_LOWER_LIMIT) &&
		    (p_test_res->open_test_result &
		     BEYOND_RAWDATA_LOWER_LIMIT) != 0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_RAWDATA_MIN_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
	...
	if ((p_test_res->open_test_result & KEY_BEYOND_UPPER_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, KEY_BEYOND_UPPER_LIMIT,
			p_test_res->p_beyond_key_upper_limit_cnt, key_num);

		if (0 == (test_res_unstb & KEY_BEYOND_UPPER_LIMIT) &&
		    (p_test_res->open_test_result & KEY_BEYOND_UPPER_LIMIT) !=
			    0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_KEYDATA_MAX_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
				RAWDATA_TEST_BEYOND_KEY_MAX_LIMIT;
		}
	...
	if ((p_test_res->open_test_result & KEY_BEYOND_LOWER_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, KEY_BEYOND_LOWER_LIMIT,
			p_test_res->p_beyond_key_lower_limit_cnt, key_num);

		if (0 == (test_res_unstb & KEY_BEYOND_LOWER_LIMIT) &&
		    (p_test_res->open_test_result & KEY_BEYOND_LOWER_LIMIT) !=
			    0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_KEYDATA_MIN_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
				RAWDATA_TEST_BEYOND_KEY_MIN_LIMIT;
		}
	...
	if ((p_test_res->open_test_result & BEYOND_OFFSET_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, BEYOND_OFFSET_LIMIT,
			p_test_res->p_beyond_offset_limit_cnt, node_num);

		if (0 == (test_res_unstb & BEYOND_OFFSET_LIMIT) &&
		    (p_test_res->open_test_result & BEYOND_OFFSET_LIMIT) != 0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_OFFSET_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
				RAWDATA_TEST_BEYOND_OFFSET_LIMIT;
		}
	...
	if ((p_test_res->open_test_result & BEYOND_UNIFORMITY_LIMIT) != 0) {
		...
		if (0 == (test_res_unstb & BEYOND_UNIFORMITY_LIMIT) &&
		    (p_test_res->open_test_result & BEYOND_UNIFORMITY_LIMIT) !=
			    0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_UNIFORMITY_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
				RAWDATA_TEST_BEYOND_UNIFORM_LIMIT;
		}
	...
	if ((p_test_res->open_test_result & BEYOND_ACCORD_LIMIT) != 0) {
		test_res_unstb |= analyze_test_item_res(
			p_test_item, BEYOND_ACCORD_LIMIT,
			p_test_res->p_beyond_accord_limit_cnt, node_num);

		if (0 == (test_res_unstb & BEYOND_ACCORD_LIMIT) &&
		    (p_test_res->open_test_result & BEYOND_ACCORD_LIMIT) != 0) {
			id_tmp = get_arr_id_from_sub_item(
				p_test_item, TP_ACCORD_TEST_ITEM_ID);
			p_test_res->item_res_header.test_item_err_code
				.ptr_err_code_set[id_tmp + 1] =
				RAWDATA_TEST_BEYOND_ACCORD_LIMIT;
		}
	...
	if (offse_and_accord_flag == 0x03) {
		...
		for (i = 0; i < node_num; i++) {
			if (get_chn_need_check(p_test_item, i)) {
				if ((p_test_res->p_beyond_accord_limit_cnt[i] &
				     0x80) != 0) {
					if ((p_test_res->p_beyond_offset_limit_cnt
						     [i] &
					     0x80) != 0) {
						tmp_accord_res |=
							BEYOND_ACCORD_LIMIT;
	...
	if (p_test_res->open_test_result & BEYOND_JITTER_LIMIT) {
		id_tmp = get_arr_id_from_sub_item(
			p_test_item, TP_RAWDATA_JITTER_TEST_ITEM_ID);
		p_test_res->item_res_header.test_item_err_code
			.ptr_err_code_set[id_tmp + 1] =
			RAWDATA_TEST_BEYOND_JITTER_LIMIT;
		return 1;
...
static s32 analyze_difftest_res(u8 cur_frame_id, PST_TEST_ITEM p_test_item)
{
	...
	PST_TP_DEV p_dev = NULL;
	get_test_item_from_proc_data(
		(PST_TEST_PROC_DATA)p_test_item->ptr_test_proc_data,
		TP_RAWDATA_TEST_ITEMS_SET_ID, &p_raw_data_test_item);
