			  ...
			  PST_TEST_PARAM p_test_param)
{
	...
	for (i = 0; i < item_num; i++) {
		...
		case TP_SHORT_TEST_ITEM_ID: {
			ret = short_test_before_func(
				(p_test_proc_data->pp_test_item_arr)[i],
				p_test_param);
		} break;

		case TP_VERSION_TEST_ITEM_ID: {
			ret = version_test_before_func(
				(p_test_proc_data->pp_test_item_arr)[i],
				p_test_param);
		} break;
		...
		case TP_CHIP_CFG_PROC_ITEM_ID: {
			ret = send_cfg_test_before_func(
				(p_test_proc_data->pp_test_item_arr)[i],
				p_test_param);
		} break;

		case TP_RECOVER_CFG_ITEM_ID: {
			ret = cfg_recover_test_before_func(
				(p_test_proc_data->pp_test_item_arr)[i]);
		} break;

		case TP_FLASH_TEST_ITEM_ID: {
			ret = flash_test_before_func(
				(p_test_proc_data->pp_test_item_arr)[i],
				p_test_param);
		} break;
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 register_test_item_finished_func(PST_TEST_PROC_DATA p_test_proc_data)
{
	s32 ret = 1;
	u16 i = 0;
	PST_TEST_ITEM *pp_test_item_arr = NULL;
	...
	for (i = 0; i < item_num; i++) {
		if ((p_test_proc_data->pp_test_item_arr)[i] != NULL) {
			switch (pp_test_item_arr[i]->test_item_id) {
				/*----------------------tp open test-----------------------*/
			case TP_RAWDATA_TEST_ITEMS_SET_ID: {
				ret = release_rawdata_test_item((
					p_test_proc_data->pp_test_item_arr)[i]);
			} break;

				/*----------------------hardware test-----------------------*/
			case TP_SHORT_TEST_ITEM_ID: {
				ret = release_short_test_item((
					p_test_proc_data->pp_test_item_arr)[i]);
			} break;

			case TP_VERSION_TEST_ITEM_ID: {
				ret = version_test_finished_func((
					p_test_proc_data->pp_test_item_arr)[i]);
			} break;

				/*----------------------other test-----------------------*/
			case TP_CHIP_CFG_PROC_ITEM_ID: {
				ret = release_cfg_test_item((
					p_test_proc_data->pp_test_item_arr)[i]);
			} break;
...
extern s32 init_test_proc_data(PST_TEST_PROC_DATA *pp_test_proc_data,
			       PST_TEST_PARAM *pp_test_param, void *p_drv_dev)
{
	s32 ret = 1;
	u16 i = 0;
	...
	if (alloc_mem_in_heap((void **)pp_test_proc_data,
			      sizeof(ST_TEST_PROC_DATA)) == 0) {
		board_print_error("test proc data malloc memory error!");
		return 0;
	}
	...
	ret = alloc_mem_in_heap((void **)&p_test_proc_data->p_dev,
				sizeof(ST_TP_DEV));
	if (0 == ret) {
	...
	ret = alloc_mem_in_heap((void **)&(p_test_proc_data->pp_test_item_arr),
	...
	for (i = 0; i < p_test_proc_data->test_item_num; i++) {
		ret = alloc_mem_in_heap(
			(void **)(&(p_test_proc_data->pp_test_item_arr[i])),
			sizeof(ST_TEST_ITEM));
		if (0 == ret) {
			return ret;
		}
		...
		(p_test_proc_data->pp_test_item_arr)[i]->cur_data.ptr_cur_data =
			NULL;
		(p_test_proc_data->pp_test_item_arr)[i]->param.ptr_param = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]->test_res.ptr_res =
			NULL;
		(p_test_proc_data->pp_test_item_arr)[i]->ptr_general_func =
			NULL;
		(p_test_proc_data->pp_test_item_arr)[i]
			->ptr_test_item_before_func = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]
			->ptr_test_item_before_func_param = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]->ptr_test_func = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]
			->ptr_test_item_finished_func = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]
			->ptr_test_item_finished_func_param = NULL;
		(p_test_proc_data->pp_test_item_arr)[i]->ptr_test_proc_data =
			p_test_proc_data;
		(p_test_proc_data->pp_test_item_arr)[i]
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 release_test_proc_data(PST_TEST_PROC_DATA *pp_test_proc_data)
{
	s32 ret = 1;
	u16 i = 0;

	PST_TEST_PROC_DATA p_test_proc_data = NULL;
	if ((pp_test_proc_data == NULL) || ((*pp_test_proc_data) == NULL)) {
		return 1;
	}
	...
	if (NULL != p_test_proc_data->p_dev) {
		if (p_test_proc_data->p_dev->chip_type != 0xffff) {
			if (p_test_proc_data->p_dev->chip_type == TP_PHOENIX) {
				ret = chip_reg_write(
					p_test_proc_data->p_dev,
					p_test_proc_data->p_dev->cmd_set
						.hid_ena_cmd.addr,
					p_test_proc_data->p_dev->cmd_set
						.hid_ena_cmd.cmd_buf,
					p_test_proc_data->p_dev->cmd_set
	...
	if (NULL != p_test_proc_data->pp_test_item_arr) {
		...
		ret = free_mem_in_heap(p_test_proc_data->pp_test_item_arr);
		p_test_proc_data->pp_test_item_arr = NULL;
	}
...
extern s32 test_process(void *p_drv_dev)
{
	...
	ret = init_test_proc_data(&p_test_proc_data, &p_test_param, p_drv_dev);
	if (ret == 0) {
		goto TEST_EXIT;
	}
	/*(2)start test*/
...
extern int get_tp_rawdata(void *p_drv_dev, char *buf, int *buf_size)
{
	...
	node_num = p_test_proc_data->p_dev->sc_sen_num *
		   p_test_proc_data->p_dev->sc_drv_num;
	data_size = get_data_size(p_test_proc_data->p_dev->rawdata_option);
	board_print_debug("[%s]node_num = %d,data_size = %d\n", __func__,
			  node_num, data_size);

	if (!alloc_mem_block((void **)&(p_cur_data), sizeof(p_cur_data))) {
		goto TEST_EXIT;
	}
	if (alloc_mem_block((void **)&(p_cur_data->p_data_buf),
			    node_num * data_size)) {
		/*init_for_buffer*/
	...
	ret = chip_mode_select(
		(p_test_proc_data->pp_test_item_arr)[rawdata_test_id]->p_tp_dev,
		CHIP_RAW_DATA_MODE);
	board_get_rawdata(
		(p_test_proc_data->pp_test_item_arr)[rawdata_test_id]->p_tp_dev,
		3000, 0, p_cur_data);
	...
	for (i = 0; i < node_num; i++) {
		data_tmp = (s16)((p_cur_data->p_data_buf[i * 2]) +
				 (p_cur_data->p_data_buf[i * 2 + 1] << 8));
		r = snprintf(&buf[offset], 10, "%4d,", data_tmp);
		offset += r;
		if (((i + 1) % (p_test_proc_data->p_dev->sc_drv_num)) == 0) {
			r = snprintf(&buf[offset], 2, "\n");
			offset += r;
		}
	}
