...
* Return		: none
*******************************************************************************/
static void get_key_pos_map(PST_TP_DEV p_tp_dev, u8 *key_pos,
			    u8 key_num_in_port, u8 drv_flag)
{
	u8 i = 0;
	u8 area_key = 0;
	u8 tmp_pos = 0;
	...
	if (area_key == 1) {
		...
		for (i = 0; i < strlen((const char *)key_pos); i++) {
			if (key_pos[i] != 0) {
				...
				p_tp_dev->key_pos_arr[key_num] = tmp_pos;
				key_num++;
			}
		}
...
*******************************************************************************/
extern s32 get_sc_drv_sen_key_num(PST_TP_DEV p_tp_dev, u8 key_start_addr,
				  u8 key_en, u8 sen_as_key, u8 key_com_port_num,
				  u8 max_key_num)
{
	s32 ret = 1;
	...
	if (sen_as_key == 0) {
		...
		if (key_com_port_num == 1) {
			...
			p_tp_dev->sc_drv_num = p_tp_dev->total_drv_num - key_en;
		}
	} else {
		if (key_com_port_num == 1) {
			max_key_num_in_com_port = max_key_num / 2;
			...
			max_key_num_in_com_port = max_key_num;
			p_tp_dev->sc_sen_num = p_tp_dev->total_sen_num - key_en;
		}
	}
	...
	if (p_tp_dev->key_rawdata_num == 0) {
		...
		/*read key_pos*/
		ret = chip_reg_read(p_tp_dev, key_start_addr, key_pos,
				    sizeof(key_pos));
		if (ret == 0) {
			board_print_error("read key_pos faild!\n");
			return 0;
		}
		get_key_pos_map(p_tp_dev, key_pos, max_key_num_in_com_port,
				drv_flag);
	}
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
static s32 get_step4_abs_addr(PST_TP_DEV p_dev, PST_ADDR_POS_INFO p_addr_info,
			      cu8 *config)
{
	s32 ret = 1;
	...
	if (i >= config[2]) {
		board_print_error("Cann't find the specifiled bag num %d\n",
	...
	p_addr_info->addr = p_dev->cfg_start_addr +
			    (sub_bag_ptr + p_addr_info->offset - config);
	return ret;
}
				 ...
				 PST_ADDR_POS_INFO p_addr2_info)
{
	...
	if (p_addr2_info != NULL) {
		field = (u8 *)strsep((char **)&str_tmp, ",");
		if (field == NULL) {
			board_print_debug("get addr2 null, parse str:%s!\n",
					  string_info);
			return ret;
		}
		p_addr2_info->addr = atohex(field, strlen((const char *)field));
				 ...
				 PST_ADDR_POS_INFO p_addr2_info)
{
	...
	if ((p_addr2_info != NULL) && (field != NULL)) {
		...
		p_addr2_info->package_id = atoi((const char *)&field[1]);
...
static s32 parse_addr_data(PST_TP_DEV p_dev, u8 *string_info,
			   PST_ADDR_POS_INFO p_addr1_info,
			   PST_ADDR_POS_INFO p_addr2_info, u8 data_step)
{
	getrid_space((s8 *)string_info, strlen((const char *)string_info));
	if (data_step == 3) {
		return parse_step3_addr_data(p_dev, string_info, p_addr1_info,
					     p_addr2_info);
	}
	if (data_step == 4) {
		return parse_step4_addr_data(p_dev, string_info, p_addr1_info,
					     p_addr2_info);
	}
	return 0;
}
...
*******************************************************************************/
static u8 get_step3_cfg_data(PST_TP_DEV p_tp_dev, PST_ADDR_POS_INFO p_addr_info,
			     cu8 *config)
{
	u8 value = 0;
	u8 tmp = 0;
	...
	tmp &= 0xff;
	value = (config[p_addr_info->addr - p_tp_dev->cfg_start_addr] >>
		 p_addr_info->lsb) &
		tmp;
	return value;
}
...
* Return		: u8(data in cfg)
*******************************************************************************/
static u8 get_step4_cfg_data(PST_TP_DEV p_tp_dev, PST_ADDR_POS_INFO p_addr_info,
			     cu8 *config)
{
	s32 ret = 0;
	u8 value = 0;
...
*******************************************************************************/
static u8 get_cfg_data(PST_TP_DEV p_tp_dev, PST_ADDR_POS_INFO p_addr_info,
		       cu8 *config, u8 data_step)
{
	if (data_step == 3) {
		return get_step3_cfg_data(p_tp_dev, p_addr_info, config);
	}
	if (data_step == 4) {
		return get_step4_cfg_data(p_tp_dev, p_addr_info, config);
	}
	return 0;
}
...
extern u32 parse_cfg_value(PST_TP_DEV p_dev, u8 *string_info, u8 *p_config)
{
	...
	val_num = parse_addr_data(p_dev, string_info, &addr_info1, &addr_info2,
				  step);
	if (val_num == 0) {
		return 0;
	}
	...
	if (val_num >= 2) {
		value2 = get_cfg_data(p_dev, &addr_info2, p_config, step);
	}
	ret = value1 + value2;
	return ret;
}
			   ...
			   u16 cfg_len)
{
	...
	num = parse_addr_data(p_tp_dev, str_info, &addr_info1, NULL, step);
	if (num == 0) {
		return 0;
	}
...
extern void init_dev_const_param(PST_TP_DEV p_tp_dev)
{
	...
	p_tp_dev->cfg_len =
		690; /*if ext_cfg_len != 0 then cfg_len = base_cfg_len + ext_cfg_len*/
	...
	/*real_cmd_addr*/
	p_tp_dev->real_cmd_addr =
		0x6F68; /*this field may be same as fields in cmd_set*/
	...
	p_tp_dev->rawdata_option = _DATA_BIT_16 | _DATA_LARGE_ENDIAN |
				   _DATA_SIGNED | _DATA_DRV_SEN_INVERT;
...
{
	s32 ret = 0;
	p_tp_dev->total_sen_num = parse_cfg_value(p_tp_dev, SEN_NUM_ADDR, cfg);
	p_tp_dev->sc_sen_num = p_tp_dev->total_sen_num;
	board_print_debug("total sen num:%d\n", p_tp_dev->total_sen_num);
	p_tp_dev->total_drv_num = parse_cfg_value(p_tp_dev, DRV_NUM_ADDR, cfg);
	p_tp_dev->sc_drv_num = p_tp_dev->total_drv_num;
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
static s32 init_test_config(PST_TP_DEV p_tp_dev, u8 *test_cfg, u16 *p_cfg_len)
{
	s32 ret = 1;
	if (*p_cfg_len == 0) {
...
* Return		: s32(0:fail 1:ok)
*******************************************************************************/
extern s32 parse_init_dev_info(PST_TP_DEV p_tp_dev, PST_TEST_PARAM p_test_param)
{
	s32 ret = 1;
	...
	obj_set = mxmlFindElement(top, top, "OBJ_SETTING", NULL, NULL,
				  MXML_DESCEND);
	...
	parent_xnode = mxmlFindElement(obj_set, obj_set, "powerVolt", NULL,
				       NULL, MXML_DESCEND);
	...
	/*find chip type*/
	parent_xnode = mxmlFindElement(obj_set, obj_set, "tpMainType", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	parent_xnode = mxmlFindElement(parent_xnode, obj_set, "tpSubType", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	chip_ini =
		mxmlFindElement(top, top, "chipIni", NULL, NULL, MXML_DESCEND);
	...
	ret = header_file_init_dev(p_tp_dev, p_test_param->test_cfg,
				   &p_test_param->cfg_len);
	return ret;
}
...
extern s32 parse_order_config(PST_TEST_PARAM p_test_param)
{
	...
	ret = alloc_mem_in_heap((void **)&p_test_param->test_cfg,
				TP_MAX_CFG_LEN);
	if (ret == 0) {
	...
	parent_xnode = mxmlFindElement(top, top, "chipConfigProc", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	/*init test config */
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "config",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode != NULL) && (child_xnode->child != NULL)) {
		ret = hexstr_to_array(p_test_param->test_cfg,
				      child_xnode->child->value.opaque,
				      TP_MAX_CFG_LEN);
		if (ret > 0) {
	...
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "option",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode != NULL) && (child_xnode->child != NULL)) {
...
extern s32 parse_test_item_id(PST_TEST_PARAM p_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "testItemSet", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	ret = alloc_mem_in_heap((void **)&p_test_param->p_test_item_id_set,
				MAX_TEST_NUM * sizeof(ST_TEST_ITEM_ID_SET));
	if (ret == 0) {
		board_print_error("test item id set malloc memory error!\n");
		return 0;
	}
	...
	while ((test_item != NULL) && (test_item->child != NULL)) {
		...
		if ((is_select != NULL) && (is_select->child != NULL) &&
		    (str_to_int(is_select->child->value.opaque) == 1)) {
			/*find test id*/
			test_id = mxmlFindElement(test_item, test_item,
						  "itemId", NULL, NULL,
						  MXML_DESCEND);
			if ((test_id != NULL) && (test_id->child != NULL)) {
				itest_id = str_to_int(
					test_id->child->value.opaque);
				if (is_mach_test_item(itest_id) != 1) {
					test_item = mxmlFindElement(
						test_item, parent_xnode,
						"testItem", NULL, NULL,
						MXML_NO_DESCEND);
					continue;
				}
				board_print_debug("test id :%d\n", itest_id);
				p_test_id_set[test_num].test_id = str_to_int(
					test_id->child->value.opaque);
				/*find sub test id*/
				sub_item = mxmlFindElement(test_item, test_item,
							   "subTestItem", NULL,
							   NULL, MXML_DESCEND);
				if ((sub_item != NULL) &&
				    (sub_item->child != NULL)) {
					sub_id_num = 0;
					ret = alloc_mem_in_heap(
						(void **)&p_test_id_set[test_num]
							.p_sub_id_set,
				...
				while ((sub_item != NULL) &&
				       (sub_item->child != NULL)) {
					is_select = mxmlFindElement(
						sub_item, sub_item, "bSelected",
						NULL, NULL, MXML_DESCEND);
					board_print_debug(
						"sub id is select:%d\n",
						str_to_int(
							is_select->child->value
								.opaque));
					if ((is_select != NULL) &&
					    (is_select->child != NULL) &&
					    (str_to_int(is_select->child->value
								.opaque) ==
					     1)) {
						/*find sub item id */
						test_id = mxmlFindElement(
							sub_item, sub_item,
							"itemId", NULL, NULL,
							MXML_DESCEND);
						if ((test_id != NULL) &&
					...
					sub_item = mxmlFindElement(
						sub_item, test_item,
						"subTestItem", NULL, NULL,
						MXML_DESCEND);
				}
				if (p_test_id_set[test_num].p_sub_id_set !=
				    NULL) {
					p_test_id_set[test_num]
						.p_sub_id_set->len = sub_id_num;
					board_print_debug("sub test num:%d\n",
							  sub_id_num);
				}
				test_num++;
			}
...
* Return		: u16(node num)
*******************************************************************************/
extern u16 parse_need_check_node(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	s32 ret = 0;
	char *tmp_value = NULL;
	u16 data_len = 0;
	...
	parent_xnode = mxmlFindElement(top, top, "needCheckNodeParam", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
	...
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_need_check_node,
				sizeof(ST_NEED_CHECK_NODE));
	if (ret == 0) {
		board_print_error("need check node malloc memory error!\n");
		return 0;
	}
	ret = alloc_mem_in_heap(
		(void **)&p_raw_test_param->p_need_check_node->p_data_buf,
		data_len);
	if (ret == 0) {
		board_print_error("need check node malloc memory error!\n");
		return 0;
	...
	memset(p_raw_test_param->p_need_check_node->p_data_buf, 0, data_len);
	p_raw_test_param->p_need_check_node->data_len =
		hexstr_to_array(p_raw_test_param->p_need_check_node->p_data_buf,
				tmp_value, data_len);
	return data_len;
}
				...
				u16 data_len)
{
	...
	ret = alloc_mem_in_heap(
		(void **)&p_raw_test_param->p_raw_key_limit->p_need_check_key,
		sizeof(ST_NEED_CHECK_KEY));
	if (ret == 0) {
		board_print_error("need check key malloc memory error!\n");
		return 0;
	}
	p_need_check_key = p_raw_test_param->p_raw_key_limit->p_need_check_key;
	/*init need check key*/
	ret = alloc_mem_in_heap((void **)&p_need_check_key->p_data_buf,
				data_len);
	if (ret == 0) {
		board_print_error("need check key buf malloc memory error!\n");
		return 0;
	}
	...
	p_need_check_key->len = hexstr_to_array(p_need_check_key->p_data_buf,
						tmp_value, data_len);
	return ret;
}
			      ...
			      u16 data_len)
{
	...
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_special_node,
				sizeof(ST_RAWDATA_SPECIAL_NODE_PARAM));
	if (ret == 0) {
		board_print_error("need check node malloc memory error!\n");
		return 0;
	}
	...
	ret = alloc_mem_in_heap(
		(void **)&p_raw_test_param->p_special_node->p_data_buf,
		data_len * sizeof(ST_SPECIAL_NODE_DEF));
	if (ret == 0) {
		board_print_error("need check node malloc memory error!\n");
		return 0;
	}
	...
	special_xnode = mxmlFindElement(top, top, "rawdataSpecialNodeParam",
					NULL, NULL, MXML_DESCEND);
	...
	i = 0;
	parent_xnode = mxmlFindElement(special_xnode, special_xnode, "node",
				       NULL, NULL, MXML_DESCEND);
	while ((special_xnode != NULL) && (special_xnode->child != NULL)) {
		/*node id */
		...
		p_raw_test_param->p_special_node->p_data_buf[i].node_id =
			str_to_int(child_xnode->child->value.opaque);

		/*high raw limit*/
		child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
					      "highRawLimit", NULL, NULL,
					      MXML_DESCEND);
		if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
			break;
		}
		p_raw_test_param->p_special_node->p_data_buf[i].high_raw_limit =
			str_to_int(child_xnode->child->value.opaque);

		/*low raw limit*/
		child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
					      "lowRawLimit", NULL, NULL,
					      MXML_DESCEND);
		if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
			break;
		}
		p_raw_test_param->p_special_node->p_data_buf[i].low_raw_limit =
			str_to_int(child_xnode->child->value.opaque);

		/*accord limit*/
		child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
					      "accordLimit", NULL, NULL,
					      MXML_DESCEND);
		if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
			break;
		}
		p_raw_test_param->p_special_node->p_data_buf[i].accord_limit =
			str_to_int(child_xnode->child->value.opaque);

		/*offset limit*/
		child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
					      "offsetLimit", NULL, NULL,
					      MXML_DESCEND);
		if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
			break;
		}
		p_raw_test_param->p_special_node->p_data_buf[i].offset_limit =
			str_to_int(child_xnode->child->value.opaque);

		/*accord option*/
		child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
					      "accordOption", NULL, NULL,
					      MXML_DESCEND);
		if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
			break;
		}
		p_raw_test_param->p_special_node->p_data_buf[i].accord_option =
			str_to_int(child_xnode->child->value.opaque);
		i++;
		parent_xnode = mxmlFindElement(parent_xnode, special_xnode,
					       "node", NULL, NULL,
					       MXML_DESCEND);
	}
...
extern s32 parse_raw_max_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "rawdataLimitParam", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	if (p_raw_test_param->p_rawdata_limit == NULL) {
		ret = alloc_mem_in_heap(
			(void **)&p_raw_test_param->p_rawdata_limit,
			sizeof(ST_RAWDATA_LIMIT_PARAM));
		if (ret == 0) {
			board_print_error(
...
extern s32 parse_raw_min_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "rawdataLimitParam", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find rawdataLimitParam in order!\n");
		return 0;
	}
...
extern s32 parse_accord_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "accordLimitParam", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find accordLimitParam in order!\n");
		return 0;
	}
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_raw_accord_limit,
				sizeof(ST_RAW_ACCORD_LIMIT_PARAM));
	if (ret == 0) {
		board_print_error("accord limit malloc memory error!\n");
		return 0;
	}
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "accordLimit",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error("can not find accordLimit in order!\n");
		return 0;
	}
	...
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error("can not find accordOption in order!\n");
		return 0;
	}
...
extern s32 parse_offset_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "offsetLimitParam", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find offsetLimitParam in order!\n");
		return 0;
	}
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_raw_offset_limit,
				sizeof(ST_RAW_OFFSET_LIMIT_PARAM));
	if (ret == 0) {
		board_print_error("offset limit malloc memory error!\n");
		return 0;
	}
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "offsetLimit",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error("can not find offsetLimit in order!\n");
		return 0;
	}
...
extern s32 parse_uniform_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "uniformLimitParam", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find uniformLimitParam in order!\n");
		return 0;
	}
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_raw_uniform_limit,
	...
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode,
				      "uniformLimit", NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error(
...
extern s32 parse_jitter_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "jitterLimitParam", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find jitterLimitParam in order!\n");
		return 0;
	}
	ret = alloc_mem_in_heap((void **)&p_raw_test_param->p_raw_jitter_limit,
				sizeof(ST_RAW_JITTER_LIMIT_PARAM));
	if (ret == 0) {
		board_print_error("jitter limit malloc memory error!\n");
		return 0;
	}
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "jitterLimit",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error(
...
extern s32 parse_key_max_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	parent_xnode = mxmlFindElement(top, top, "keydataLimitParam", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find keydataLimitParam in order!\n");
		return 0;
	}
	if (p_raw_test_param->p_raw_key_limit == NULL) {
		ret = alloc_mem_in_heap(
			(void **)&p_raw_test_param->p_raw_key_limit,
	...
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "upperLimit",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error("can not find key upperLimit in order!\n");
		return 0;
	}
...
extern s32 parse_key_min_limit(PST_RAWDATA_TEST_PARAM p_raw_test_param)
{
	...
	child_xnode = mxmlFindElement(parent_xnode, parent_xnode, "lowerLimit",
				      NULL, NULL, MXML_DESCEND);
	if ((child_xnode == NULL) || (child_xnode->child == NULL)) {
		board_print_error("can not find key lowerLimit in order!\n");
		return 0;
	}
				...
				PST_SUB_TEST_ITEM_ID_SET p_sub_id_set)
{
	...
	if ((test_item == NULL) || (test_item->child == NULL)) {
		board_print_error("can not find rawdataTestParam in order!\n");
		return 0;
	}
	...
	if (ret == 0) {
		board_print_error("raw test param malloc memory error!\n");
		return ret;
	}
	p_raw_test_param =
	...
	for (i = 0; i < p_sub_id_set->len; i++) {
		...
		case TP_RAWDATA_MAX_TEST_ITEM_ID: {
			/*rawdata limit*/
			ret = parse_raw_max_limit(p_raw_test_param);
			break;
		}
		case TP_RAWDATA_MIN_TEST_ITEM_ID: {
			ret = parse_raw_min_limit(p_raw_test_param);
			break;
		}
		case TP_ACCORD_TEST_ITEM_ID: {
			/*accord limit*/
			ret = parse_accord_limit(p_raw_test_param);
			break;
		}
		case TP_OFFSET_TEST_ITEM_ID: {
			/*offset limit*/
			ret = parse_offset_limit(p_raw_test_param);
			break;
		}
		case TP_UNIFORMITY_TEST_ITEM_ID: {
			/*uniform limit*/
			ret = parse_uniform_limit(p_raw_test_param);
			break;
		}
		case TP_RAWDATA_JITTER_TEST_ITEM_ID: {
			/*jitter limit*/
			ret = parse_jitter_limit(p_raw_test_param);
			break;
		}
		case TP_KEYDATA_MAX_TEST_ITEM_ID: {
			/*key data upper limit*/
			ret = parse_key_max_limit(p_raw_test_param);
			break;
		}
		case TP_KEYDATA_MIN_TEST_ITEM_ID: {
			/*key data min limit*/
			ret = parse_key_min_limit(p_raw_test_param);
			break;
		}
		default: {
			break;
		...
		if (ret == 0) {
			return ret;
			board_print_error("parse rawdata item error!\n");
		}
	}
	/*need check node*/
	...
	/*special test node*/
	ret = parse_special_node(p_raw_test_param, data_len + TP_MAX_KEY_NUM);
	if (ret == 0) {
...
extern s32 parse_short_test_param(PST_TEST_PARAM p_test_param)
{
	...
	test_item = mxmlFindElement(top, top, "shortThresholdParam", NULL, NULL,
				    MXML_DESCEND);
	if ((test_item == NULL) || (test_item->child == NULL)) {
		board_print_error(
	...
	if (ret == 0) {
		board_print_error("short test param malloc memory error!\n");
		return 0;
	}
	...
	parent_xnode = mxmlFindElement(test_item, test_item,
				       "drvGndVddResistorThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.drv_gnd_vdd_resistor_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"drv_gnd_vdd_resistor_threshold is :%d\n",
		p_short_param->short_threshold.drv_gnd_vdd_resistor_threshold);

	parent_xnode = mxmlFindElement(test_item, test_item,
				       "senGndVddResistorThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.sen_gnd_vdd_resistor_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"sen_gnd_vdd_resistor_threshold is :%d\n",
		p_short_param->short_threshold.sen_gnd_vdd_resistor_threshold);

	parent_xnode = mxmlFindElement(test_item, test_item,
				       "senSenResistorThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.sen_sen_resistor_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"sen_sen_resistor_threshold is :%d\n",
		p_short_param->short_threshold.sen_sen_resistor_threshold);

	parent_xnode = mxmlFindElement(test_item, test_item,
				       "drvDrvResistorThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.drv_drv_resistor_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"drv_drv_resistor_threshold is :%d\n",
		p_short_param->short_threshold.drv_drv_resistor_threshold);

	parent_xnode = mxmlFindElement(test_item, test_item,
				       "drvSenResistorThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.drv_sen_resistor_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"drv_sen_resistor_threshold is :%d\n",
		p_short_param->short_threshold.drv_sen_resistor_threshold);

	parent_xnode = mxmlFindElement(test_item, test_item, "gtShortThreshold",
				       NULL, NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	parent_xnode = mxmlFindElement(test_item, test_item, "adcReadDelay",
				       NULL, NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	parent_xnode = mxmlFindElement(test_item, test_item,
				       "diffcodeShortThreshold", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_short_param->short_threshold.diffcode_short_threshold =
		str_to_int(parent_xnode->child->value.opaque);
	board_print_debug(
		"diffcode_short_threshold is :%d\n",
		p_short_param->short_threshold.diffcode_short_threshold);

	p_short_param->short_threshold.tx_tx_factor = 68;
	parent_xnode = mxmlFindElement(test_item, test_item, "txTxFactor", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode != NULL) && (parent_xnode->child != NULL)) {
	...
	parent_xnode = mxmlFindElement(test_item, test_item, "txRxFactor", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode != NULL) && (parent_xnode->child != NULL)) {
	...
	parent_xnode = mxmlFindElement(test_item, test_item, "rxRxFactor", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode != NULL) && (parent_xnode->child != NULL)) {
...
extern s32 parse_version_test_param(PST_TEST_PARAM p_test_param)
{
	...
	if (p_test_param->p_version_test_param == NULL) {
		ret = alloc_mem_in_heap(
			(void **)&p_test_param->p_version_test_param,
	...
	test_item = mxmlFindElement(top, top, "versionTestParam", NULL, NULL,
				    MXML_DESCEND);
	if ((test_item == NULL) || (test_item->child == NULL)) {
		board_print_error(
	...
	parent_xnode = mxmlFindElement(test_item, test_item, "icVerAddr", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_ver_param->ic_ver_addr =
		atohex((unsigned char *)parent_xnode->child->value.opaque,
		       strlen((const char *)parent_xnode->child->value.opaque));
	board_print_debug("ic version addr:0x%04x!\n",
			  p_ver_param->ic_ver_addr);
	p_ver_param->b_auto_update = 0;

	parent_xnode = mxmlFindElement(test_item, test_item, "versionStr", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error(
	...
	p_ver_param->ver_param_len = hexstr_to_array(
		p_ver_param->ver_param_str, parent_xnode->child->value.opaque,
		MAX_VERSION_LEN);
...
extern s32 parse_flash_test_param(PST_TEST_PARAM p_test_param)
{
	...
	/*find version test param*/
	ret = alloc_mem_in_heap((void **)&p_test_param->p_flash_test_param,
				sizeof(ST_FLASH_TEST_PARAM));
	if (ret == 0) {
		board_print_error("flash test param malloc memory error!\n");
		return 0;
	}
	...
	test_item = mxmlFindElement(top, top, "flashTestParam", NULL, NULL,
				    MXML_DESCEND);
	if ((test_item == NULL) || (test_item->child == NULL)) {
		board_print_error(
	...
	parent_xnode = mxmlFindElement(test_item, test_item, "option", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
		board_print_error("can not find flash option,order error!\n");
		return 0;
	}
	p_flash_param->option = str_to_int(parent_xnode->child->value.opaque);
	board_print_debug("flash test option:0x%04x!\n", p_flash_param->option);

	parent_xnode = mxmlFindElement(test_item, test_item, "delayTime", NULL,
				       NULL, MXML_DESCEND);
	if ((parent_xnode != NULL) && (parent_xnode->child != NULL)) {
...
static s32 parse_ic_name(PST_TEST_PARAM p_test_param)
{
	...
	parent_xnode = mxmlFindElement(obj_set, obj_set, "chipName", NULL, NULL,
				       MXML_DESCEND);
	if ((parent_xnode == NULL) || (parent_xnode->child == NULL)) {
...
extern s32 parse_test_param(PST_TEST_PARAM *pp_test_param)
{
	...
	if (alloc_mem_in_heap((void **)pp_test_param, sizeof(ST_TEST_PARAM)) ==
	    0) {
	...
	for (i = 0; i < p_test_param->test_item_num; i++) {
		...
		case TP_RAWDATA_TEST_ITEMS_SET_ID: {
			...
			need_send_cfg++;
			ret = parse_raw_test_param(
				p_test_param,
				p_test_param->p_test_item_id_set[i]
					.p_sub_id_set);
			break;
		}
		case TP_SHORT_TEST_ITEM_ID: {
			need_send_cfg++;
			ret = parse_short_test_param(p_test_param);
			break;
		}
		case TP_VERSION_TEST_ITEM_ID: {
			ret = parse_version_test_param(p_test_param);
			break;
		}
		case TP_FLASH_TEST_ITEM_ID: {
			ret = parse_flash_test_param(p_test_param);
			break;
		}
	...
	if (need_send_cfg > 0) {
		if ((cfg_flag & 0x01) == 0) {
			p_test_param
				->p_test_item_id_set[p_test_param->test_item_num]
		...
		if (((cfg_flag & 0x01) == 1) && ((cfg_flag & 0x02) == 0)) {
			p_test_param
				->p_test_item_id_set[p_test_param->test_item_num]
				.test_id = TP_RECOVER_CFG_ITEM_ID;
			p_test_param->test_item_num++;
...
extern s32 release_order_config(PST_TEST_PARAM p_test_param)
{
	...
	if (p_test_param->test_cfg != NULL) {
		ret = free_mem_in_heap_p((void **)&p_test_param->test_cfg);
	}
...
extern s32 release_test_item_id(PST_TEST_PARAM p_test_param)
{
	...
	if (p_test_param->p_test_item_id_set != NULL) {
		if (p_test_param->p_test_item_id_set[i].p_sub_id_set != NULL) {
			if (p_test_param->p_test_item_id_set[i]
				    .p_sub_id_set->p_id_set != NULL) {
				ret = free_mem_in_heap_p(
					(void **)&p_test_param
						->p_test_item_id_set[i]
						.p_sub_id_set->p_id_set);
			}
			ret = free_mem_in_heap_p(
				(void **)&p_test_param->p_test_item_id_set[i]
					.p_sub_id_set);
		}
		ret = free_mem_in_heap_p(
			(void **)&p_test_param->p_test_item_id_set);
	}
...
extern s32 release_raw_test_param(PST_TEST_PARAM p_test_param)
{
	...
	if (p_raw_test_param != NULL) {
		...
		if (p_raw_test_param->p_need_check_node != NULL) {
			ret = free_mem_in_heap_p(
				(void **)&p_raw_test_param->p_need_check_node
					->p_data_buf);
			ret = free_mem_in_heap_p(
				(void **)&p_raw_test_param->p_need_check_node);
		}
		...
		if (p_raw_test_param->p_special_node != NULL) {
			ret = free_mem_in_heap_p(
				(void **)&p_raw_test_param->p_special_node
					->p_data_buf);
			ret = free_mem_in_heap_p(
				(void **)&p_raw_test_param->p_special_node);
		}
...
extern s32 release_flash_test_param(PST_TEST_PARAM p_test_param)
{
	...
	if (p_flash_param != NULL) {
		if (p_flash_param->config_arr != NULL) {
			ret = free_mem_in_heap(
				(void *)p_flash_param->config_arr);
		}
...
*******************************************************************************/
static s32 decrypt_tp_order(PST_TP_DEV p_tp_dev, s8 **pp_buf, u32 *p_len)
{
	s32 ret = 0;
	s8 *ver = NULL;
...
*******************************************************************************/
extern int parse_test_order(PST_TEST_PARAM *pp_test_param, PST_TP_DEV p_tp_dev)
{
	s32 ret = 1;
	...
	if (p_test_param->p_raw_test_param != NULL &&
	    p_test_param->p_raw_test_param->p_need_check_node != NULL) {
		if (p_test_param->p_raw_test_param->p_need_check_node->data_len <
