				  ...
				  PST_TEST_PARAM p_test_param)
{
	...
	if (alloc_mem_in_heap((void **)&p_short_param->short_opt.opt.opt_ptr,
			      p_short_param->short_opt.len) == 0) {
		board_print_error("short testopt param malloc momory error!\n");
		return 0;
	}
	...
	*/
	...
	opt_param[i++] = ((p_short_param->short_threshold
	...
	opt_param[i++] =
		((p_short_param->short_threshold.gt_short_threshold >> 8) &
		 0xFF);
	/*diffcode threshold*/
	opt_param[i++] =
	...
	opt_param[i++] = ((p_short_param->short_threshold.tx_tx_factor) & 0xFF);
	opt_param[i++] =
		((p_short_param->short_threshold.tx_tx_factor >> 8) & 0xFF);
	/*Tx与Rx短路换算系数，范围40~500*/
	opt_param[i++] = ((p_short_param->short_threshold.tx_rx_factor) & 0xFF);
	opt_param[i++] =
		((p_short_param->short_threshold.tx_rx_factor >> 8) & 0xFF);
	/*Rx与Rx短路换算系数，范围40~500*/
	opt_param[i++] = ((p_short_param->short_threshold.rx_rx_factor) & 0xFF);
	opt_param[i++] =
	...
	for (j = 3; j < len + 3; j += 2) {
		chk_sum += ((u16)(opt_param[j + 1] << 8) + opt_param[j]);
	}
	chk_sum = 0 - chk_sum;
			   ...
			   PST_TEST_PARAM p_test_param)
{
	...
	case TP_NANJING:
	case TP_PHOENIX: {
		ret = get_phoenix_test_param(p_test_item, p_test_param);
	} break;
	default: {
		ret = get_gt900_short_param(p_test_item, p_test_param);
	} break;
	}
	return ret;
}
...
extern s32 release_short_test_item(IN_OUT ptr32 p_data)
{
	...
	if (p_short_param != NULL) {
		...
		if (p_short_param->short_opt.opt.opt_ptr != NULL) {
			free_mem_in_heap(p_short_param->short_opt.opt.opt_ptr);
...
extern s32 version_test_before_func(ptr32 p_data, ptr32 p_param)
{
	...
	p_test_item->test_res.ptr_res = NULL;
	/*init version param*/
	p_test_item->param.ptr_param = p_test_param->p_version_test_param;
...
extern s32 release_flash_test_item(ptr32 p_data)
{
	...
	PST_FLASH_TEST_PARAM p_flash_param = NULL;
	p_test_item = (PST_TEST_ITEM)p_data;
	p_flash_param = (PST_FLASH_TEST_PARAM)p_test_item->param.ptr_param;
...
extern s32 send_cfg_test_before_func(ptr32 p_data, ptr32 p_param)
{
	...
	if (ret == 0) {
		board_print_error("get send config test result error!\n");
		return 0;
	}
	return ret;
}
				   ...
				   PST_TEST_PARAM p_test_param)
{
	...
	ret = alloc_mem_in_heap((void **)&p_send_cfg_param,
				sizeof(ST_CHIP_CONFIG_PROC_PARAM));
	if (ret == 0) {
		board_print_error("send config param alloc memory error!\n");
		return 0;
	}
...
extern s32 release_send_cfg_param(PST_TEST_ITEM p_test_item)
{
	...
	if (p_send_cfg_param != NULL) {
		...
		ret = free_mem_in_heap_p(
			(void **)&p_test_item->param.ptr_param);
		p_test_item->param.ptr_param = NULL;
	}
	return ret;
}
...
extern s32 cfg_recover_test_before_func(ptr32 p_data)
{
	...
	if (ret == 0) {
		board_print_error("get cfg recover test result error!\n");
		return 0;
	}
	p_res = (PST_RECOVER_CHIP_CFG_RES)p_test_item->test_res.ptr_res;
	return ret;
}
...
extern s32 get_recover_cfg_param_mess(PST_TEST_ITEM p_test_item)
{
	...
	ret = alloc_mem_in_heap((void **)&p_recover_cfg_param,
